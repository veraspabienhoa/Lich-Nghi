# V86.0 - Popup thống kê nghỉ + bỏ nhân sự nghỉ việc khỏi hạn mức + ngoại lệ Leader/Đám hiếu + V85.9 features (2026-08-19)
...
def add_source_leave_type_column(df):
    """V85.7: thêm Thứ ngày sau Ngày và Loại nghỉ sau Lý do nghỉ cho các bảng lịch nghỉ."""
    if not isinstance(df, pd.DataFrame):
        return df
    ...
    if not daily_thuc_nghi.empty:
        ...
        for _idx, _stat in daily_stats_df.iterrows():
            ...
            if not _day_thuc.empty:
                _rn = _day_thuc["Lý do nghỉ"].astype(str).str.strip().str.lower()
                _co_rows = _day_thuc[(_rn != "nghỉ phát sinh") & (~_rn.str.contains("không phép", na=False))].copy()
                _ps_rows = _day_thuc[_rn == "nghỉ phát sinh"].copy()
                _kp_rows = _day_thuc[_rn.str.contains("không phép", na=False)].copy()
            else:
