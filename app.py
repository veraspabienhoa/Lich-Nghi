# V87.7 - Fix Tạm hoãn Vi phạm không cập nhật bảng lương lịch sử + V87.6 features (2026-08-20)
...
UI_THEME_GROUP_ORDER = [
    ...
    "metric", "tabs", "sidebar_menu", "card_box", "page_layout",
]
...
def _ui_device_defaults(size, height, px, py, radius, text="#333333", bg="#FFFFFF",
...
def _global_theme_rules_for_device(cfg, device):
    ...
    selectors = {
        ...
        "card_box": "[data-testid='stAlert'],[data-testid='stVerticalBlockBorderWrapper'],div[data-testid='stForm'],[data-testid='stPopover']",
    }
...
def render_global_ui_theme_css(config=None):
    ...
    st.markdown(
        f"""
<style id="vera-global-theme-v87">
{desktop_css}
@media (max-width:768px) {{
{mobile_css}
}}
...
def _theme_editor_column_config():
    return {
        ...
        "Font": st.column_config.SelectboxColumn("Font", options=UI_THEME_FONT_OPTIONS, width=130),
        "Cỡ chữ": st.column_config.NumberColumn("Cỡ chữ", min_value=8, max_value=48, step=1, format="%d", width=80),
        "Cao": st.column_config.NumberColumn(
            "Cao", min_value=0, max_value=120, step=1, format="%d", width=70,
            help="Chiều cao tối thiểu (px). Áp dụng cho Header VERA SPA, nút, ô nhập, tab và menu. 0 = tự động."
        ),
        "Padding X": st.column_config.NumberColumn("Padding X", min_value=0, max_value=40, step=1, format="%d", width=85),
        "Padding Y": st.column_config.NumberColumn("Padding Y", min_value=0, max_value=30, step=1, format="%d", width=85),
        "Bo góc": st.column_config.NumberColumn("Bo góc", min_value=0, max_value=30, step=1, format="%d", width=80),
        "Khoảng cách": st.column_config.NumberColumn("Khoảng cách", min_value=0, max_value=30, step=1, format="%d", width=95),
        "Màu chữ": st.column_config.TextColumn("Màu chữ", width=95, help="HEX #RRGGBB"),
        "Màu nền": st.column_config.TextColumn("Màu nền", width=95, help="HEX #RRGGBB"),
...
def render_admin_theme_config_panel():
    ...
    with st.expander("🎨 Giao diện toàn hệ thống · Desktop / Mobile", expanded=True):
        st.caption(
            ...
            "expander, metric, tab, MENU CHỨC NĂNG, card/box và khoảng cách trang. "
    ...
    # V87.1: bảng 6 cột thống kê theo ngày có cấu hình riêng.
