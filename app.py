# V85.5 - TimeSoft là nguồn dữ liệu lương mặc định + V85.4 features (2026-08-19)
...
def _standardize_payroll_source_from_timesoft(invoice_df):
    """
    Chuyển báo cáo doanh thu TimeSoft về đúng cấu trúc mà build_payroll_table đang dùng:
    Thời gian / Sản phẩm-Dịch vụ-PT / Tổng tiền / NV tư vấn.

    Ưu tiên frame/cột có dòng bắt đầu bằng 'Tip'. Nếu TimeSoft đổi tên field nhẹ,
    bộ dò theo schema + dữ liệu sẽ tự nhận diện lại.
    """
    ...
    for frame_name, frame in _timesoft_expand_payroll_candidate_frames(invoice_df):
        ...
        e_col, e_score = _timesoft_pick_payroll_column(frame, "employee")

        # Nếu API giữ đúng thứ tự tương tự file dữ liệu lương cũ, dùng B/F/G/I làm fallback.
        if frame.shape[1] >= 9:
        ...
        out = pd.DataFrame({
            "Thời gian": frame[t_col],
            "Sản phẩm/ Dịch vụ/ PT": frame[i_col],
            "Tổng tiền": frame[m_col],
            "NV tư vấn": frame[e_col],
        }).replace({None: ""})
        ...
        out["NV tư vấn"] = out["NV tư vấn"].astype(str).str.strip()
        out["Sản phẩm/ Dịch vụ/ PT"] = out["Sản phẩm/ Dịch vụ/ PT"].astype(str).str.strip()
        out = out.dropna(subset=["Thời gian_DT"]).copy()

        if out.empty:
            continue

        tip_mask = out["Sản phẩm/ Dịch vụ/ PT"].str.casefold().str.startswith("tip")
        ...
        # Ưu tiên tuyệt đối frame có nhiều dòng Tip hợp lệ.
        quality = (
            tip_count * 1000
            + tip_valid_emp * 100
            + tip_nonzero_money * 100
    ...
    if best is None:
        ...
        return pd.DataFrame(columns=base_cols), (
            "Không tự nhận diện được cấu trúc dữ liệu lương từ TimeSoft. "
            f"Các cột TimeSoft nhận được: {cols_preview}"
        )

    # Lưu thông tin kỹ thuật an toàn để Admin có thể kiểm tra mapping nếu cần.
