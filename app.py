# V91.3 - Tách quyền dùng Đơn nghỉ dài hạn và quyền xem Thống kê nghỉ dài hạn (2026-08-20)
...
FEATURE_DEFINITIONS = {
    ...
    "leave_detail_edit": "✏️ Sửa trực tiếp Chi tiết danh sách · Đăng ký lịch nghỉ",
    "leave_manage_edit": "✏️ Sửa trực tiếp Chi tiết danh sách · Quản lý lịch nghỉ",
    "leave_today_khong_phep_edit_delete": "🗓️ Sửa/Xóa KHÔNG PHÉP ngày hiện tại",
    "employee_penalty_view": "💰 Xem tiền phạt vi phạm nhân viên",
    "long_leave": "🏖️ Lịch nghỉ dài hạn",
    "long_leave_form": "📝 Sử dụng mẫu Đơn xin nghỉ dài hạn",
    "long_leave_stats": "📊 Xem danh sách / thống kê nghỉ dài hạn",
...
DEFAULT_ROLE_FEATURES = {
    ...
    "quanly": {"tour", "leave", "leave_manage", "leave_detail_edit", "leave_manage_edit", "long_leave", "long_leave_stats", "shift", "staff_list", "employee_add", "employee_edit", "employment_status", "employee_delete", "profile", "birthday"},
    "letan": {"tour", "leave", "leave_manage", "leave_detail_edit", "leave_manage_edit", "long_leave", "long_leave_stats", "shift", "staff_list", "employee_add", "employee_edit", "employment_status", "employee_delete", "profile", "birthday"},
    "leader": {"tour", "leave", "leave_manage", "long_leave", "long_leave_form", "profile", "birthday"},
    "nhanvien": {"tour", "leave", "leave_manage", "long_leave", "long_leave_form", "profile", "birthday"},
    "locker": {"tour", "long_leave", "profile", "birthday"},
    "tapvu": {"long_leave", "birthday"},
}
...
def _long_leave_request_id(employee_name):
    raw = (
        f"{normalize_login_name(employee_name)}|"
        f"{datetime.now(VN_TZ).isoformat()}|{secrets.token_hex(4)}"
    )
    return "LL-" + hashlib.sha1(raw.encode("utf-8")).hexdigest()[:12].upper()
...
def append_long_leave_request(
    ...
    try:
        ...
        if ws is None:
            return False, "Không kết nối được Google Sheets.", ""
        now = datetime.now(VN_TZ)
        req_id = _long_leave_request_id(employee_name)
        row = [
...
def update_long_leave_row(row_idx, updates, updated_by):
    try:
        ...
        _clear_long_leave_cache()
        return True, "Đã cập nhật yêu cầu nghỉ dài hạn."
    except Exception as e:
...
def delete_long_leave_row(row_idx):
    try:
        ...
        _clear_long_leave_cache()
        return True, "Đã xóa bản ghi nghỉ dài hạn."
    except Exception as e:
...
def upload_long_leave_document(uploaded_file, request_id, uploaded_by):
    ...
    try:
        ...
        meta = {
            ...
            "description": (
                f"VERA SPA - Tài liệu nghỉ dài hạn {request_id} - "
...
def attach_documents_to_long_leave_request(request_id, uploaded_files, uploaded_by):
    ...
    added, errors = [], []
    for f in files:
        ok, msg, record = upload_long_leave_document(f, request_id, uploaded_by)
        if ok and record:
            added.append(record)
        else:
            errors.append(msg)
    ...
    return True, f"Đã tải {len(added)} tài liệu.", added
...
def send_long_leave_request_email(
    ...
    try:
        ...
        <p><b>Thời gian:</b> {start_date.strftime('%d/%m/%Y')} - {end_date.strftime('%d/%m/%Y')}</p>
        <p><b>Lý do:</b> {html.escape(str(reason))}</p>
...
def send_long_leave_document_reminder(employee_name, request_id, end_date):
    ...
    try:
        ...
        msg["From"] = f"Vera Spa <{sender_email}>"
        msg["To"] = to_email
        msg["Subject"] = f"[VERA SPA] Nhắc tải tài liệu nghỉ dài hạn - {employee_name}"
        msg.attach(MIMEText(
            f"""
            <html><body style='font-family:Arial,sans-serif'>
        ...
        server.send_message(msg)
        server.quit()
        return True, "Đã gửi email nhắc tải tài liệu."
    except Exception as e:
...
def save_long_leave_request_pause(enabled, username, message=""):
    try:
        ...
        try:
            ...
            pass
        return True, "Đã cập nhật trạng thái nhận đơn nghỉ dài hạn."
    except Exception as e:
...
def can_view_employee_penalty():
    ...
    if role == "quanly": return has_feature_access("employee_penalty_view")
    return False
...
SHIFT_CONFIG_HEADERS = ["ID", "Tên ca", "Giờ bắt đầu", "Giờ kết thúc", "Ghi chú", "Thứ tự", "Trạng thái"]
...
def _get_shift_config_worksheet():
    ...
    ss = client.open_by_key(SHEET_MAT_KHAU_ID)
    ws = _get_or_create_worksheet(
        ss, SHIFT_CONFIG_WORKSHEET, rows=300, cols=len(SHIFT_CONFIG_HEADERS)
    )
    vals = _gs_call_with_backoff(ws.get_all_values)
    if not vals:
        gspread_update_range(ws, "A1:G1", [SHIFT_CONFIG_HEADERS])
        defaults = [
            ["SHIFT001", "Ca 1", "10:00", "23:00", "", "1", "Đang dùng"],
            ["SHIFT002", "Ca 2", "13:00", "00:00", "", "2", "Đang dùng"],
            ["SHIFT003", "Cố định Ca 1", "", "", "Không đổi", "3", "Đang dùng"],
            ["SHIFT004", "Cố định Ca 2", "", "", "Không đổi", "4", "Đang dùng"],
        ]
        for row in defaults:
            _gs_call_with_backoff(ws.append_row, row, value_input_option="USER_ENTERED")
    elif vals[0][:len(SHIFT_CONFIG_HEADERS)] != SHIFT_CONFIG_HEADERS:
        gspread_update_range(ws, "A1:G1", [SHIFT_CONFIG_HEADERS])
    return ws
...
def load_shift_definitions():
    ...
    if not rows:
        rows = [
            {"ID":"SHIFT001","Tên ca":"Ca 1","Giờ bắt đầu":"10:00","Giờ kết thúc":"23:00","Ghi chú":"","Thứ tự":1,"Trạng thái":"Đang dùng"},
            {"ID":"SHIFT002","Tên ca":"Ca 2","Giờ bắt đầu":"13:00","Giờ kết thúc":"00:00","Ghi chú":"","Thứ tự":2,"Trạng thái":"Đang dùng"},
            {"ID":"SHIFT003","Tên ca":"Cố định Ca 1","Giờ bắt đầu":"","Giờ kết thúc":"","Ghi chú":"Không đổi","Thứ tự":3,"Trạng thái":"Đang dùng"},
            {"ID":"SHIFT004","Tên ca":"Cố định Ca 2","Giờ bắt đầu":"","Giờ kết thúc":"","Ghi chú":"Không đổi","Thứ tự":4,"Trạng thái":"Đang dùng"},
        ]
    return pd.DataFrame(rows).sort_values(["Thứ tự", "Tên ca"], kind="stable").reset_index(drop=True)
...
def get_shift_options():
    ...
    if isinstance(d, pd.DataFrame) and not d.empty:
        active = d[d["Trạng thái"].astype(str).str.strip().str.lower().ne("đã xóa")]
        for _, r in active.iterrows():
            label = _shift_display_label(r.get("Tên ca",""), r.get("Giờ bắt đầu",""), r.get("Giờ kết thúc",""))
            if str(r.get("Ghi chú","")).strip().lower() == "không đổi" and "không đổi" not in label.lower():
                label = f"{label} (Không đổi)"
            if label and label not in opts:
                opts.append(label)
    return opts or list(SHIFT_OPTIONS)
...
def save_shift_definition(shift_id, name, start_time, end_time, note, order, username):
    name = str(name or "").strip()
    if not name:
    ...
    try:
        ...
        if shift_id and isinstance(defs, pd.DataFrame) and not defs.empty:
            ...
            if not hit.empty:
                ...
                old_label = _shift_display_label(rr["Tên ca"], rr["Giờ bắt đầu"], rr["Giờ kết thúc"])
                if str(rr.get("Ghi chú","")).strip().lower() == "không đổi" and "không đổi" not in old_label.lower():
                    old_label += " (Không đổi)"
        if not shift_id:
            shift_id = "SHIFT-" + datetime.now(VN_TZ).strftime("%Y%m%d%H%M%S")
        row = [
            shift_id, name, str(start_time or "").strip(), str(end_time or "").strip(),
            str(note or "").strip(), str(int(order or 999)), "Đang dùng"
        ]
        if row_idx:
            gspread_update_range(ws, f"A{row_idx}:G{row_idx}", [row])
        else:
            _gs_call_with_backoff(ws.append_row, row, value_input_option="USER_ENTERED")
        try: load_shift_definitions.clear()
        except Exception: pass
        new_label = _shift_display_label(name, start_time, end_time)
        if str(note or "").strip().lower() == "không đổi" and "không đổi" not in new_label.lower():
            new_label += " (Không đổi)"
        ...
        _clear_dynamic_data_caches()
        return True, "Đã lưu ca làm việc."
    except Exception as e:
...
def delete_shift_definition(shift_id, username):
    try:
        ...
        _clear_dynamic_data_caches()
        return ok_replace, ("Đã xóa ca. " + msg_replace).strip()
    except Exception as e:
...
def build_shift_template_excel_bytes(credentials_df):
    ...
    _shift_options = get_shift_options()
...
def validate_schedule_delete_permission(original_row, role, current_user=None, today=None):
    ...
    if role in {"letan", "quanly"}:
        ...
        # V90: quyền riêng chỉ cho KHÔNG PHÉP đúng ngày hiện tại.
        ...
        if not _has_general_leave_edit:
            return False, (
                "Tài khoản chưa được cấp quyền xóa lịch. Admin có thể cấp quyền "
                "'Sửa/Xóa KHÔNG PHÉP ngày hiện tại' hoặc quyền sửa trực tiếp lịch nghỉ."
            )

        if is_video_leave_reason(old_reason) or is_bereavement_leave_reason(old_reason):
            return True, ""
        if old_date is not None and old_date < today:
...
def validate_schedule_edit_permission(original_row, edited_row, role, today=None, current_user=None):
    ...
    if role in EMPLOYEE_LIKE_ROLES:
        ...
        # V90: quyền riêng cho phép sửa đúng dòng KHÔNG PHÉP của ngày hiện tại.
        # Không cho đổi ngày hoặc chuyển sang nhóm nghỉ khác bằng quyền này.
        special_today_khong_phep = (
            has_feature_access("leave_today_khong_phep_edit_delete")
            and old_date == today
            and new_date == today
        ...
        if not special_today_khong_phep:
            _has_general_leave_edit = (
                has_feature_access("leave_manage_edit")
                or has_feature_access("leave_detail_edit")
            )
            if not _has_general_leave_edit:
                return False, (
                    "Tài khoản chưa được cấp quyền sửa lịch. Admin có thể cấp quyền "
                    "'Sửa/Xóa KHÔNG PHÉP ngày hiện tại' hoặc quyền sửa trực tiếp lịch nghỉ."
                )
            special_frontdesk = (
...
UI_THEME_GROUP_LABELS = {
    "brand_header": "🏷️ Header VERA SPA",
    "main_title": "Tiêu đề lớn",
    "sub_title": "Tiêu đề con",
    "small_title": "Tiêu đề nhỏ / Expander title",
    ...
    "tabs": "Tabs",
    "sidebar_menu": "MENU CHỨC NĂNG",
    "card_box": "Card / Box / Alert / Container",
    "tooltip_popover": "Tooltip / Popover",
    "scrollbar": "Thanh cuộn / Scrollbar",
    "modal_dialog": "Modal / Dialog",
    "page_layout": "Khoảng cách toàn trang",
}
...
UI_THEME_DEFAULT = {
    ...
    "small_title": {
        "desktop": _ui_device_defaults(18, 0, 8, 4, 6, "#222222", "#D9D9D9"),
        "mobile":  _ui_device_defaults(18, 0, 6, 3, 6, "#222222", "#D9D9D9"),
    },
    "label_content": {
        "desktop": _ui_device_defaults(16, 0, 6, 3, 5, "#333333", "#D9D9D9"),
        "mobile":  _ui_device_defaults(16, 0, 4, 2, 5, "#333333", "#D9D9D9"),
    },
    "table": {
        "desktop": _ui_device_defaults(13, 0, 4, 3, 0, "#333333", "#D9D9D9"),
        "mobile":  _ui_device_defaults(12, 0, 2, 2, 0, "#333333", "#D9D9D9"),
    },
    "button": {
        "desktop": _ui_device_defaults(16, 40, 12, 6, 8, "#222222", "#FFFFFF"),
    ...
    "expander": {
        ...
        "mobile":  _ui_device_defaults(18, 0, 7, 5, 8, "#222222", "#D9D9D9"),
    },
    "metric": {
        "desktop": _ui_device_defaults(16, 0, 8, 5, 8, "#333333", "#FFFFFF"),
        "mobile":  _ui_device_defaults(15, 0, 5, 3, 7, "#333333", "#FFFFFF"),
    },
    "tabs": {
        "desktop": _ui_device_defaults(16, 38, 10, 5, 6, "#333333", "#FFFFFF"),
        "mobile":  _ui_device_defaults(14, 34, 5, 3, 6, "#333333", "#FFFFFF"),
    },
    "sidebar_menu": {
        "desktop": _ui_device_defaults(16, 42, 8, 5, 8, "#222222", "#FFFFFF"),
        "mobile":  _ui_device_defaults(16, 42, 7, 5, 8, "#222222", "#FFFFFF"),
    },
    "card_box": {
...
def render_daily_summary_ui_editor():
    ...
    with st.expander("📅 Bảng Thống kê chi tiết theo từng ngày", expanded=False):
        st.caption(
            "Chỉnh riêng bảng 6 cột. Desktop dùng px; Mobile dùng tỷ lệ và tự chuẩn hóa "
            "để toàn bộ cột vừa 100% màn hình. Cột Tổng tiền phạt chỉ hiển thị với Admin."
        )
        if err:
            st.warning(err)
        ...
        col_edit = st.data_editor(
            ...
            key="daily_summary_columns_v871",
            hide_index=True,
            num_rows="fixed",
            width="stretch",
            height="content",
            disabled=["Cột"],
            column_config={
                "Cột": st.column_config.TextColumn("Cột", disabled=True, width=175),
                "Desktop (px)": st.column_config.NumberColumn(
                    "Desktop (px)", min_value=55, max_value=500, step=5, format="%d", width=110
                ),
                "Mobile (tỷ lệ)": st.column_config.NumberColumn(
                    "Mobile (tỷ lệ)", min_value=1.0, max_value=100.0, step=1.0, format="%.0f", width=115,
                    help="Trọng số chiều rộng. App tự chuẩn hóa thành 100% theo số cột thực tế."
                ),
            },
        )
        preview = _daily_summary_apply_column_editor(current, col_edit)

        desktop_tab, mobile_tab = st.tabs(["🖥️ Desktop", "📱 Mobile"])

        def _edit_style(device, prefix):
            out = dict(preview["style"][device])
            settings = [
                ("header_font", "Cỡ chữ tiêu đề", 6, 30),
        ...
        with desktop_tab:
            preview["style"]["desktop"] = _edit_style("desktop", "daily_summary_desktop_v871")
        with mobile_tab:
            preview["style"]["mobile"] = _edit_style("mobile", "daily_summary_mobile_v871")

        b1, b2 = st.columns(2)
        with b1:
            if st.button("💾 Lưu bảng thống kê ngày", use_container_width=True, key="save_daily_summary_ui_v871"):
                ok, msg = save_daily_summary_ui_config(preview, st.session_state.current_user)
                (st.success if ok else st.error)(msg)
                if ok:
                    rerun_current_view()
        with b2:
            if st.button("♻️ Khôi phục mặc định", use_container_width=True, key="reset_daily_summary_ui_v871"):
...
def render_data_table_builder():
    ...
    with st.expander("📊 Data Table · Header / Body / Rows / Grid", expanded=False):
        st.caption("Áp dụng toàn hệ thống. Cấu hình riêng Desktop và Mobile.")
        ...
        def editor(device):
            ...
            a,b,c1 = st.columns(3)
            with a:
                c["header_bg"] = st.text_input("Header nền", c["header_bg"], key=f"dt_{device}_hbg")
                c["header_text"] = st.text_input("Header chữ", c["header_text"], key=f"dt_{device}_ht")
                c["header_weight"] = st.selectbox("Header Weight", UI_THEME_WEIGHT_OPTIONS, index=UI_THEME_WEIGHT_OPTIONS.index(c["header_weight"]), key=f"dt_{device}_hw")
                c["sticky_header"] = st.toggle("Sticky Header", value=c["sticky_header"], key=f"dt_{device}_sticky")
                c["body_bg"] = st.text_input("Body nền", c["body_bg"], key=f"dt_{device}_bbg")
                c["body_text"] = st.text_input("Body chữ", c["body_text"], key=f"dt_{device}_bt")
            with b:
                c["zebra_enabled"] = st.toggle("Zebra Stripes", value=c["zebra_enabled"], key=f"dt_{device}_zebra")
                c["zebra_bg"] = st.text_input("Zebra màu", c["zebra_bg"], key=f"dt_{device}_zbg")
                c["row_hover_bg"] = st.text_input("Row Hover màu", c["row_hover_bg"], key=f"dt_{device}_rh")
                c["grid_horizontal"] = st.toggle("Grid ngang", value=c["grid_horizontal"], key=f"dt_{device}_gh")
                c["grid_vertical"] = st.toggle("Grid dọc", value=c["grid_vertical"], key=f"dt_{device}_gv")
                c["border_color"] = st.text_input("Màu đường kẻ", c["border_color"], key=f"dt_{device}_bc")
            with c1:
                c["border_width"] = st.number_input("Độ dày lưới", 0, 5, int(c["border_width"]), key=f"dt_{device}_bw")
                c["cell_padding_x"] = st.number_input("Cell Padding X", 0, 30, int(c["cell_padding_x"]), key=f"dt_{device}_px")
                c["cell_padding_y"] = st.number_input("Cell Padding Y", 0, 30, int(c["cell_padding_y"]), key=f"dt_{device}_py")
                c["text_align"] = st.selectbox("Text Align", UI_THEME_ALIGN_OPTIONS, index=UI_THEME_ALIGN_OPTIONS.index(c["text_align"]), key=f"dt_{device}_ta")
                c["number_align"] = st.selectbox("Number/Money Align", UI_THEME_ALIGN_OPTIONS, index=UI_THEME_ALIGN_OPTIONS.index(c["number_align"]), key=f"dt_{device}_na")
                c["status_align"] = st.selectbox("Status/Icon Align", UI_THEME_ALIGN_OPTIONS, index=UI_THEME_ALIGN_OPTIONS.index(c["status_align"]), key=f"dt_{device}_sa")
            edited[device] = c
        with t1: editor("desktop")
        with t2: editor("mobile")
        x1,x2 = st.columns(2)
        with x1:
            if st.button("💾 Lưu Data Table", use_container_width=True, key="save_dt_v88"):
                ok,msg = save_data_table_ui_config(edited, st.session_state.current_user)
                (st.success if ok else st.error)(msg)
                if ok: rerun_current_view()
        with x2:
            if st.button("♻️ Khôi phục Data Table", use_container_width=True, key="reset_dt_v88"):
...
def render_compact_admin_preset_builder():
    ...
    with st.expander("🧩 Preset · Bảng quản trị gọn", expanded=False):
        st.caption(
            "Preset đang dùng phong cách bạn đã chọn: bảng gọn, header xám nhạt, body trắng, "
            "viền mảnh, hover nhẹ. Có thể chỉnh trực tiếp từng thông số rồi Lưu / Áp dụng toàn app."
        )
        if err:
            st.warning(err)
        ...
        def _device_editor(device):
            c = dict(edited[device])
            a, b, d = st.columns(3)
            with a:
                c["font_size"] = st.number_input("Body font", 8, 24, int(c["font_size"]), key=f"compact_{device}_fs")
                c["header_font_size"] = st.number_input("Header font", 8, 24, int(c["header_font_size"]), key=f"compact_{device}_hfs")
                c["header_weight"] = st.selectbox(
                    "Header weight", UI_THEME_WEIGHT_OPTIONS,
                    index=UI_THEME_WEIGHT_OPTIONS.index(c["header_weight"]),
                    key=f"compact_{device}_hw"
                )
                c["header_bg"] = st.text_input("Header nền", c["header_bg"], key=f"compact_{device}_hbg")
                c["header_text"] = st.text_input("Header chữ", c["header_text"], key=f"compact_{device}_ht")
                c["body_bg"] = st.text_input("Body nền", c["body_bg"], key=f"compact_{device}_bbg")
                c["body_text"] = st.text_input("Body chữ", c["body_text"], key=f"compact_{device}_bt")
            with b:
                c["zebra_bg"] = st.text_input("Zebra", c["zebra_bg"], key=f"compact_{device}_zb")
                c["row_hover_bg"] = st.text_input("Row Hover", c["row_hover_bg"], key=f"compact_{device}_rh")
                c["border_color"] = st.text_input("Màu viền", c["border_color"], key=f"compact_{device}_bc")
                c["border_width"] = st.number_input("Viền px", 0, 5, int(c["border_width"]), key=f"compact_{device}_bw")
                c["cell_padding_x"] = st.number_input("Cell Padding X", 0, 20, int(c["cell_padding_x"]), key=f"compact_{device}_px")
                c["cell_padding_y"] = st.number_input("Cell Padding Y", 0, 20, int(c["cell_padding_y"]), key=f"compact_{device}_py")
                c["radius"] = st.number_input("Bo góc", 0, 24, int(c["radius"]), key=f"compact_{device}_radius")
            with d:
                c["button_height"] = st.number_input("Nút cao", 28, 80, int(c["button_height"]), key=f"compact_{device}_bh")
                c["button_font"] = st.number_input("Nút font", 8, 24, int(c["button_font"]), key=f"compact_{device}_bf")
                c["input_height"] = st.number_input("Input cao", 28, 80, int(c["input_height"]), key=f"compact_{device}_ih")
                c["expander_header_bg"] = st.text_input("Expander nền", c["expander_header_bg"], key=f"compact_{device}_ebg")
                c["expander_font"] = st.number_input("Expander font", 8, 28, int(c["expander_font"]), key=f"compact_{device}_ef")
                c["metric_font"] = st.number_input("Metric font", 10, 36, int(c["metric_font"]), key=f"compact_{device}_mf")
            edited[device] = c

        with t1:
            _device_editor("desktop")
        with t2:
            _device_editor("mobile")

        c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("💾 Lưu preset", use_container_width=True, key="save_compact_preset_v882"):
        ...
        with c2:
            if st.button("✨ Áp dụng toàn app", use_container_width=True, key="apply_compact_preset_v882"):
                # Lưu preset trước, sau đó ghi sang Theme + Data Table.
                ...
                if not ok_p:
                    ...
                    if ok_t and ok_d:
                        st.success("✅ Đã áp dụng preset Bảng quản trị gọn cho toàn bộ app.")
                        rerun_current_view()
                    else:
                        st.error(" | ".join([m for ok, m in [(ok_t,msg_t),(ok_d,msg_d)] if not ok]))

        with c3:
            if st.button("♻️ Reset preset", use_container_width=True, key="reset_compact_preset_v882"):
...
def _save_button_default_config():
    ...
    return {"desktop":one(), "mobile":one(), "labels":{
        "generic":"💾 Lưu", "confirm_leave":"💾 Xác Nhận Ghi Lịch Nghỉ",
        "save_all":"💾 Lưu tất cả thay đổi", "save_default":"💾 Lưu làm mặc định",
        "overwrite":"💾 Ghi đè cập nhật bản lương này",
    }}

def _normalize_save_button_config(raw=None):
    base=_save_button_default_config(); raw=raw if isinstance(raw,dict) else {}
    for device in ["desktop","mobile"]:
        incoming=raw.get(device,{}) if isinstance(raw.get(device,{}),dict) else {}; out=base[device]
        font=str(incoming.get("font_family",out["font_family"])); out["font_family"]=font if font in UI_THEME_FONT_OPTIONS else out["font_family"]
        weight=str(incoming.get("font_weight",out["font_weight"])); out["font_weight"]=weight if weight in UI_THEME_WEIGHT_OPTIONS else out["font_weight"]
        style=str(incoming.get("font_style",out["font_style"])); out["font_style"]=style if style in SAVE_BUTTON_STYLE_OPTIONS else out["font_style"]
        shadow=str(incoming.get("shadow",out["shadow"])); out["shadow"]=shadow if shadow in UI_THEME_SHADOW_OPTIONS else out["shadow"]
        for ck in ["text_color","bg_color","border_color","hover_bg","hover_text"]: out[ck]=_valid_theme_hex(incoming.get(ck),out[ck])
        for k,(lo,hi) in {"font_size":(8,30),"border_width":(0,8),"radius":(0,30),"width_px":(0,1200),"height_px":(28,120)}.items():
            try: out[k]=max(lo,min(hi,int(float(incoming.get(k,out[k])))))
            except Exception: pass
    labels=raw.get("labels",{}) if isinstance(raw.get("labels",{}),dict) else {}
    for key in base["labels"]:
        val=str(labels.get(key,base["labels"][key])).strip(); base["labels"][key]=val or base["labels"][key]
    return base
...
def render_save_button_ui_builder():
    if st.session_state.get("current_role")!="admin": return
    current,err=load_save_button_ui_config()
    with st.expander("💾 Nút Lưu / Save · Giao diện & tên hiển thị",expanded=False):
        st.caption("Áp dụng cho nút Lưu, Save, Ghi đè, Xác nhận ghi. Chỉnh riêng Desktop/Mobile và đổi tên hiển thị.")
        if err: st.warning(err)
        edited=json.loads(json.dumps(current)); td,tm=st.tabs(["🖥️ Desktop","📱 Mobile"])
        def edit_device(device):
            c=dict(edited[device]); a,b,d=st.columns(3)
            with a:
                c["font_family"]=st.selectbox("Font chữ",UI_THEME_FONT_OPTIONS,index=UI_THEME_FONT_OPTIONS.index(c["font_family"]),key=f"savebtn_{device}_font")
                c["font_size"]=st.number_input("Size chữ",8,30,int(c["font_size"]),key=f"savebtn_{device}_size")
                c["font_weight"]=st.selectbox("Độ đậm",UI_THEME_WEIGHT_OPTIONS,index=UI_THEME_WEIGHT_OPTIONS.index(c["font_weight"]),key=f"savebtn_{device}_weight")
                c["font_style"]=st.selectbox("Kiểu chữ",SAVE_BUTTON_STYLE_OPTIONS,index=SAVE_BUTTON_STYLE_OPTIONS.index(c["font_style"]),key=f"savebtn_{device}_style")
                c["text_color"]=st.text_input("Màu chữ",c["text_color"],key=f"savebtn_{device}_text")
            with b:
                c["bg_color"]=st.text_input("Màu nền",c["bg_color"],key=f"savebtn_{device}_bg")
                c["border_color"]=st.text_input("Màu viền",c["border_color"],key=f"savebtn_{device}_border")
                c["border_width"]=st.number_input("Độ dày viền",0,8,int(c["border_width"]),key=f"savebtn_{device}_bw")
                c["radius"]=st.number_input("Bo góc",0,30,int(c["radius"]),key=f"savebtn_{device}_radius")
                c["shadow"]=st.selectbox("Đổ bóng",UI_THEME_SHADOW_OPTIONS,index=UI_THEME_SHADOW_OPTIONS.index(c["shadow"]),key=f"savebtn_{device}_shadow")
            with d:
                c["width_px"]=st.number_input("Rộng px (0 = tự động)",0,1200,int(c["width_px"]),key=f"savebtn_{device}_width")
                c["height_px"]=st.number_input("Cao px",28,120,int(c["height_px"]),key=f"savebtn_{device}_height")
                c["hover_bg"]=st.text_input("Hover nền",c["hover_bg"],key=f"savebtn_{device}_hoverbg")
                c["hover_text"]=st.text_input("Hover chữ",c["hover_text"],key=f"savebtn_{device}_hovertext")
            edited[device]=c
        with td: edit_device("desktop")
        with tm: edit_device("mobile")
        st.markdown("**Đặt lại tên hiển thị trên nút bấm**"); l1,l2=st.columns(2)
        with l1:
            edited["labels"]["generic"]=st.text_input("Nút Lưu / Save chung",edited["labels"]["generic"],key="savebtn_label_generic")
            edited["labels"]["confirm_leave"]=st.text_input("Xác nhận ghi lịch nghỉ",edited["labels"]["confirm_leave"],key="savebtn_label_leave")
            edited["labels"]["save_all"]=st.text_input("Lưu tất cả thay đổi",edited["labels"]["save_all"],key="savebtn_label_all")
        with l2:
            edited["labels"]["save_default"]=st.text_input("Lưu làm mặc định",edited["labels"]["save_default"],key="savebtn_label_default")
            edited["labels"]["overwrite"]=st.text_input("Ghi đè bảng lương",edited["labels"]["overwrite"],key="savebtn_label_overwrite")
        b1,b2=st.columns(2)
        with b1:
            if st.button("💾 Lưu cấu hình nút Save",use_container_width=True,key="save_save_button_ui_v89"):
                ok,msg=save_save_button_ui_config(edited,st.session_state.current_user); (st.success if ok else st.error)(msg)
                if ok: rerun_current_view()
        with b2:
            if st.button("♻️ Khôi phục nút Save mặc định",use_container_width=True,key="reset_save_button_ui_v89"):
                ok,msg=save_save_button_ui_config(_save_button_default_config(),st.session_state.current_user); (st.success if ok else st.error)(msg)
                if ok: rerun_current_view()


def render_admin_theme_config_panel():
    """V87: cấu hình toàn bộ giao diện, tách Desktop/Mobile."""
    if st.session_state.get("current_role") != "admin":
        return

    # V89: preset + cấu hình riêng nút Lưu/Save.
    render_compact_admin_preset_builder()
    render_save_button_ui_builder()

    current, err = load_ui_theme_config()
    with st.expander("🎨 Giao diện toàn hệ thống · Desktop / Mobile", expanded=True):
        st.caption(
            "Áp dụng cho toàn bộ app: Header VERA SPA, tiêu đề, nội dung, bảng lớn/nhỏ, nút, ô nhập, selectbox, "
            "expander, metric, tab, MENU CHỨC NĂNG, card/box, Tooltip/Popover, Scrollbar, Modal/Dialog và khoảng cách trang. "
            "Phần cấu hình cột từng bảng bên dưới vẫn dùng để chỉnh riêng thứ tự/độ rộng từng cột."
        )
        if err:
            st.warning(err)

        tab_desktop, tab_mobile = st.tabs(["🖥️ Desktop", "📱 Mobile"])

        with tab_desktop:
            desktop_edit = st.data_editor(
                _theme_editor_df(current, "desktop"),
                key="admin_global_theme_desktop_v87",
        ...
        with tab_mobile:
            mobile_edit = st.data_editor(
                ...
                key="admin_global_theme_mobile_v87",
                hide_index=True, num_rows="fixed", width="stretch", height="content",
                disabled=["Nhóm"], row_height=40,
                column_config=_theme_editor_column_config(),
            )
        ...
        st.markdown(
            ...
            "Nút Mobile – Xem trước</div>",
            unsafe_allow_html=True,
        )

        c1, c2 = st.columns(2)
        with c1:
            if st.button("💾 Lưu toàn bộ giao diện", use_container_width=True, key="save_global_theme_v87"):
                ok, msg = save_ui_theme_config(preview_cfg, st.session_state.current_user)
                (st.success if ok else st.error)(msg)
                if ok:
                    rerun_current_view()
        with c2:
            if st.button("♻️ Khôi phục giao diện mặc định", use_container_width=True, key="reset_global_theme_v87"):
                ok, msg = save_ui_theme_config(UI_THEME_DEFAULT, st.session_state.current_user)
                (st.success if ok else st.error)(msg)
                if ok:
                    rerun_current_view()

    # V88: Data Table toàn hệ thống + bảng 6 cột thống kê theo ngày có cấu hình riêng.
...
TABLE_LAYOUT_STATIC_COLUMNS = {
    ...
    "long_leave_summary": [
        "Tên nhân viên", "Tổng số nghỉ", "Thời gian bắt đầu nghỉ", "Thời gian kết thúc nghỉ",
        "Lý do nghỉ dài hạn", "Chi tiết"
    ],
    "long_leave_daily": [
        "Ngày", "Thứ ngày", "Tên nhân viên", "Bắt đầu nghỉ", "Kết thúc nghỉ",
        "Lý do nghỉ dài hạn", "Chi tiết"
    ],
}
...
def get_table_layout(table_key, available_columns):
    available = [str(c) for c in available_columns]
    _captured = _transient_drag_layout(table_key, available)
    if _captured:
        return _captured
    ...
    with st.expander("⭐ Lưu bố cục bảng này làm mặc định", expanded=False):
        ...
        (function(){{
          try {{
            ...
            function capture(){{const grid=nearestGrid();if(!grid)return null;let hs=Array.from(grid.querySelectorAll('[role="columnheader"]')).map(h=>({{name:clean(h.innerText||h.textContent),rect:h.getBoundingClientRect()}})).filter(x=>x.name&&x.rect.width>20);if(!hs.length)return null;hs.sort((a,b)=>a.rect.left-b.rect.left);const order=[],widths={{}};hs.forEach(x=>{{const match=expected.find(c=>clean(c)===x.name||x.name.includes(clean(c)));if(match&&!order.includes(match)){{order.push(match);widths[match]=Math.round(x.rect.width);}}}});if(!order.length)return null;expected.forEach(c=>{{if(!order.includes(c))order.push(c);}});return {{order:order,widths:widths}};}}
            ...
            if(!W[lk]){{W[lk]=true;D.addEventListener('mouseup',function(){{setTimeout(function(){{const now=capture();if(!now)return;const prev=W.sessionStorage.getItem(sk)||'',curr=snap(now);if(!prev){{W.sessionStorage.setItem(sk,curr);return;}}if(curr!==prev){{W.sessionStorage.setItem(sk,curr);const u=new URL(W.location.href);u.searchParams.set(key,curr);W.location.replace(u.toString());}}}},180);}},true);}}
          }}catch(e){{}}
        }})();
        </script>
        """,height=0,width=0)
...
def _timesoft_exact_excel_layout_candidate(frame):
    """
    V90.8 - Nhận diện đúng layout Excel xuất từ:
    /Report/ReportSummaryInvoice/Index

    Quy tắc người dùng xác nhận:
      - Header ở DÒNG 3 (Excel row 3; DataFrame index 2 nếu header=None)
      - B = Thời gian
      - F = Sản phẩm/ Dịch vụ/ PT
      - G = Tổng tiền
      - I = NV tư vấn

    Hàm cũng chấp nhận header nằm trong 10 dòng đầu để chịu được biến thể report,
    nhưng ưu tiên tuyệt đối khi F của dòng 3 đúng "Sản phẩm/ Dịch vụ/ PT".
    """
    if not isinstance(frame, pd.DataFrame) or frame.empty or frame.shape[1] < 9:
        return None
    ...
    def _norm(v):
        return re.sub(
            r"[^a-z0-9]+",
            "",
            remove_vietnamese_accents(str(v or "")).casefold()
        )

    target_f = _norm("Sản phẩm/ Dịch vụ/ PT")
    target_b = _norm("Thời gian")
    target_g = _norm("Tổng tiền")
    target_i = _norm("NV tư vấn")

    # Trường hợp DataFrame đã dùng dòng 3 làm tên cột.
    cols = list(frame.columns)
    if len(cols) >= 9:
        if _norm(cols[5]) == target_f:
            data = frame.copy()
            raw = pd.DataFrame([list(cols)] + data.astype(object).values.tolist())
            out = _standardize_payroll_source(raw)
            if not out.empty:
                return {
                    "out": out,
                    "excel_like": raw,
                    ...
                    "exact_row3": True,
                    "mapping": {
                        "B · Thời gian": str(cols[1]),
                        "F · Sản phẩm/ Dịch vụ/ PT": str(cols[5]),
                        "G · Tổng tiền": str(cols[6]),
                        "I · NV tư vấn": str(cols[8]),
                    },
                }

    # Trường hợp header còn nằm bên trong dữ liệu.
    max_scan = min(10, len(frame))
    best = None
    for ridx in range(max_scan):
        row = frame.iloc[ridx].tolist()
        if len(row) < 9:
            continue

        b_ok = _norm(row[1]) == target_b
        f_ok = _norm(row[5]) == target_f
        g_ok = _norm(row[6]) == target_g
        i_norm = _norm(row[8])
        i_ok = (
            i_norm == target_i
            or "nvtuvan" in i_norm
            or ("nhanvien" in i_norm and "tuvan" in i_norm)
        )
        ...
        if i_ok:
            score += 300
        if ridx == 2:  # Excel row 3
            score += 5000

        # Giữ nguyên các dòng trước header để _standardize_payroll_source tự tìm header.
        ...
        tip_mask = (
            ...
            .astype(str).str.strip().str.casefold().str.startswith("tip")
        )
        tip_count = int(tip_mask.sum())
        score += tip_count * 10000

        candidate = {
            "score": score,
            "out": out,
            "excel_like": raw,
            "header_row_excel": ridx + 1,
            "exact_row3": ridx == 2,
            "mapping": {
                "B · Thời gian": "Cột B",
...
def _standardize_payroll_source_from_timesoft(invoice_df):
    ...
    # V90.8: ưu tiên tuyệt đối layout Excel thật do TimeSoft export:
    # header dòng 3, B/F/G/I; đặc biệt F = "Sản phẩm/ Dịch vụ/ PT".
    for frame_name, frame in _timesoft_expand_payroll_candidate_frames(invoice_df):
        exact = _timesoft_exact_excel_layout_candidate(frame)
        if exact is not None and int(exact.get("tip_count", 0) or 0) > 0:
            exact_out = exact["out"]
            known_keys = _timesoft_payroll_known_employee_keys()
            tip_mask = (
                exact_out["Sản phẩm/ Dịch vụ/ PT"]
                .astype(str).str.strip().str.casefold().str.startswith("tip")
            )
            tip_match_count = 0
            if known_keys and bool(tip_mask.any()):
                tip_match_count = int(
                    exact_out.loc[tip_mask, "NV tư vấn"].astype(str)
                    .apply(normalize_employee_match_name).isin(known_keys).sum()
                )

            best = {
                "quality": 10**9 + int(exact.get("score", 0)),
                "frame_name": f"{frame_name} · Excel header dòng {exact.get('header_row_excel', 3)}",
                "out": exact_out,
                "excel_like": exact["excel_like"],
                "mapping": exact["mapping"],
                "tip_count": int(exact.get("tip_count", 0) or 0),
                "tip_match_count": tip_match_count,
                "exact_excel_layout": True,
                "header_row_excel": exact.get("header_row_excel", 3),
            }
            # Nếu đúng chính xác dòng 3 thì không cần dò schema API nữa.
            if bool(exact.get("exact_row3", False)):
                break

    # Chỉ dò schema động nếu chưa tìm được đúng layout Excel dòng 3.
    if best is None or not best.get("exact_excel_layout"):
        for frame_name, frame in _timesoft_expand_payroll_candidate_frames(invoice_df):
            if not isinstance(frame, pd.DataFrame) or frame.empty:
                continue
    ...
    if int(best.get("tip_count", 0) or 0) <= 0:
        return pd.DataFrame(columns=base_cols), (
            ...
            "(Header dòng 3; B=Thời gian, F=Sản phẩm/ Dịch vụ/ PT, G=Tổng tiền, I=NV tư vấn). "
    ...
    st.session_state["payroll_timesoft_mapping_v855"] = {
        ...
        "format": (
            f"Excel TimeSoft · Header dòng {best.get('header_row_excel', 3)} · B/F/G/I"
            if best.get("exact_excel_layout")
            else "Excel Báo cáo doanh thu hóa đơn A:K · B/F/G/I"
        ),
    }
...
def _violation_debt_state_from_ledger(ledger_df, start_date, end_date, employee_names=None):
    """
    Tính due_map / deferred_current_map trực tiếp từ một snapshot ledger đã truyền vào.
    Dùng cho các thao tác vừa ghi Google Sheet để tránh đọc lại cache PostgreSQL cũ.
    """
    ...
    cs_period, ce_period = _canonicalize_payroll_period(start_date, end_date)

    allowed = None
    if employee_names is not None:
        allowed = {
            normalize_login_name(x)
            for x in employee_names
    ...
    for _, r in d.iterrows():
        ...
        # Khoản tạm hoãn là thuộc tính cố định của kỳ phát sinh.
        # Kể cả sau này nghĩa vụ đã hoàn thành ở kỳ sau, kỳ cũ vẫn phải giảm đúng số đã hoãn.
        if (
            normalize_login_name(debt_type)
            == normalize_login_name("Tạm hoãn vi phạm")
            and src_start
            and src_end
        ):
            src_cs, src_ce = _canonicalize_payroll_period(src_start, src_end)
            if src_cs == cs_period and src_ce == ce_period:
                deferred_current_map[key] = (
                    deferred_current_map.get(key, 0.0) + amount
                )

        # Chỉ nghĩa vụ còn mở mới được đưa vào "Vi phạm kỳ trước" của kỳ hiện tại.
        if not _is_open_violation_debt_status(r.get("Trạng thái", "")):
            continue

        if due_from and due_from <= cs_period:
    ...
    active_df = (
        ...
        if active_rows
        else pd.DataFrame(columns=VIOLATION_DEBT_HEADERS)
    )
...
def get_violation_debt_state(start_date, end_date, employee_names=None):
    ...
    """
    return _violation_debt_state_from_ledger(
        load_violation_debt_ledger(),
        start_date,
        end_date,
        employee_names,
    )
...
def apply_live_violation_state_to_payroll(
    payroll_df,
    start_date,
    end_date,
    leave_primary=None,
    violation_ledger_df=None,
):
    """
    Đồng bộ CHỈ 2 cột Vi phạm của bảng lương:
      - Tiền phạt trong tháng = phạt gốc của kỳ - phần đã tạm hoãn của chính kỳ.
      - Vi phạm kỳ trước = nghĩa vụ cũ đã tới hạn.
    Sau đó tính lại Thực nhận.

    violation_ledger_df cho phép truyền snapshot Google Sheet đọc trực tiếp
    ngay sau một thao tác ghi, tránh cache PostgreSQL chưa kịp cập nhật.
    """
    if payroll_df is None or not isinstance(payroll_df, pd.DataFrame) or payroll_df.empty:
        return payroll_df.copy() if isinstance(payroll_df, pd.DataFrame) else pd.DataFrame()
    ...
    leave_df = (
        leave_primary.copy()
        if isinstance(leave_primary, pd.DataFrame)
        else load_backup_sheet_data()
    )

    penalty_map = _period_penalty_by_employee(
        start_date,
        end_date,
        leave_df,
        None,
    ...
    if isinstance(violation_ledger_df, pd.DataFrame):
        due_map, deferred_map, _ = _violation_debt_state_from_ledger(
            violation_ledger_df,
            start_date,
            end_date,
            names,
        ...
        due_map, deferred_map, _ = get_violation_debt_state(
            start_date,
            end_date,
            names,
        )
    ...
    for idx, row in d.iterrows():
        key = normalize_login_name(row.get("Tên Hệ thống", ""))
        if not key:
            continue

        raw_penalty = float(_money_to_float(penalty_map.get(key, 0)))
        deferred = float(_money_to_float(deferred_map.get(key, 0)))

        if "Tiền phạt trong tháng" in d.columns:
            d.at[idx, "Tiền phạt trong tháng"] = max(
                0.0,
                raw_penalty - deferred,
            )

        d.at[idx, "Vi phạm kỳ trước"] = float(
            _money_to_float(due_map.get(key, 0))
        )

    d = recalculate_payroll_net(d)
    return _filter_real_payroll_rows(d)



def refresh_saved_payroll_from_system(payroll_df, start_date, end_date, credentials_df=None, leave_primary=None, violation_ledger_df=None):
    """
    ...
    if isinstance(violation_ledger_df, pd.DataFrame):
        due_violation_debt_map, deferred_current_violation_map, _ = _violation_debt_state_from_ledger(
            violation_ledger_df,
            start_date,
            end_date,
            _refresh_employee_names,
        )
    else:
        due_violation_debt_map, deferred_current_violation_map, _ = get_violation_debt_state(
            start_date,
...
# ==========================================================
# V87.4 - CÔNG CỤ NGHĨA VỤ VI PHẠM CHO LỊCH SỬ BẢNG LƯƠNG
# Mục tiêu: khi mở/sửa một bản lương đã lưu, Admin vẫn có đầy đủ:
# - Kiểm tra nợ Thực nhận âm
# - Xem chi tiết từng kỳ
# - Kiểm tra khoản Admin chủ động tạm hoãn
# - Tạm hoãn thêm Vi phạm sang kỳ kế tiếp
# - Sửa / Xóa nghĩa vụ Vi phạm đang mở
# Sau thay đổi: tính lại bản lịch sử đang mở nhưng CHƯA tự ghi đè snapshot.
# ==========================================================
def _history_refresh_after_violation_change(
    payroll_df,
    start_date,
    end_date,
    hist_refresh_key,
    hist_editor_version_key,
):
    """Làm mới bản lịch sử trong session sau khi ledger NoViPham thay đổi."""
    try:
        _clear_violation_debt_cache()
    except Exception:
        pass
    ...
    # V87.7: đọc trực tiếp Google Sheet NoViPham sau khi vừa ghi.
    # Không dùng snapshot PG/cache ở lượt này để số tạm hoãn phản ánh ngay lập tức.
    fresh_violation_ledger = _load_violation_debt_ledger_from_sheets()
    ...
    refreshed_df, refresh_meta = refresh_saved_payroll_from_system(
        ...
        violation_ledger_df=fresh_violation_ledger,
    )
    st.session_state[hist_refresh_key] = _filter_real_payroll_rows(refreshed_df.copy())
    st.session_state[hist_editor_version_key] = int(
        st.session_state.get(hist_editor_version_key, 0) or 0
    ) + 1
    return refreshed_df, refresh_meta


def render_admin_open_violation_debt_overview(key_prefix="history"):
    """
    Cùng chức năng 'Kiểm tra nghĩa vụ Vi phạm còn mở' của tab Tính lương,
    nhưng key độc lập để dùng an toàn trong từng bản Lịch sử.
    """
    if str(st.session_state.get("current_role", "")).strip().lower() != "admin":
        return

    show_key = f"{key_prefix}_show_open_violation_debts"
    ...
    with _debt_btn_col:
        if st.button(
            ...
            key=f"{key_prefix}_check_open_violation_debts",
        ):
            _clear_violation_debt_cache()
    ...
    with _debt_hide_col:
        if st.button(
            ...
            use_container_width=True,
            key=f"{key_prefix}_hide_open_violation_debts",
            disabled=not bool(st.session_state.get(show_key, False)),
        ):
    ...
    _negative_total = (
        int(round(_negative_debt_summary["Tổng còn nợ"].apply(_money_to_float).sum()))
        if not _negative_debt_summary.empty else 0
    )
    _deferred_total = (
        int(round(_deferred_debt_summary["Tổng tạm hoãn"].apply(_money_to_float).sum()))
        if not _deferred_debt_summary.empty else 0
    )

    _m1, _m2, _m3, _m4 = st.columns(4)
    _m1.metric("NV nợ Thực nhận âm", len(_negative_debt_summary))
    _m2.metric("Tổng nợ Thực nhận âm", f"{_negative_total:,.0f} đ".replace(",", "."))
    _m3.metric("NV Admin tạm hoãn", len(_deferred_debt_summary))
    _m4.metric("Tổng Admin tạm hoãn", f"{_deferred_total:,.0f} đ".replace(",", "."))
    ...
    if _negative_debt_summary.empty:
        ...
        if "Tổng còn nợ" in _summary_show.columns:
            _summary_show["Tổng còn nợ"] = _summary_show["Tổng còn nợ"].apply(
                lambda x: f"{_money_to_float(x):,.0f}".replace(",", ".")
            )
        st.dataframe(_summary_show, hide_index=True, width="stretch", height="content")
        ...
        with st.expander("🔎 XEM CHI TIẾT TỪNG KỲ NỢ THỰC NHẬN ÂM", expanded=False):
            ...
            if "Số tiền" in _detail_show.columns:
                _detail_show["Số tiền"] = _detail_show["Số tiền"].apply(
                    lambda x: f"{_money_to_float(x):,.0f}".replace(",", ".")
                )
            st.dataframe(_detail_show, hide_index=True, width="stretch", height="content")
    ...
    if _deferred_debt_summary.empty:
        ...
        _deferred_show = _deferred_debt_summary.copy()
        if "Tổng tạm hoãn" in _deferred_show.columns:
            _deferred_show["Tổng tạm hoãn"] = _deferred_show["Tổng tạm hoãn"].apply(
                lambda x: f"{_money_to_float(x):,.0f}".replace(",", ".")
            )
        st.dataframe(_deferred_show, hide_index=True, width="stretch", height="content")
        ...
        with st.expander("🔎 XEM CHI TIẾT TỪNG KỲ ADMIN ĐÃ TẠM HOÃN", expanded=False):
            ...
            if "Số tiền" in _deferred_detail_show.columns:
                _deferred_detail_show["Số tiền"] = _deferred_detail_show["Số tiền"].apply(
                    lambda x: f"{_money_to_float(x):,.0f}".replace(",", ".")
                )
            st.dataframe(
...
def render_admin_history_violation_management(
    payroll_df,
    start_date,
    end_date,
    batch,
    ...
    """
    Bổ sung vào bản lương lịch sử chức năng Tạm hoãn + Sửa/Xóa Nghĩa vụ
    giống tab Tính lương. Thay đổi ledger rồi tính lại bản đang mở trong session.
    """
    if str(st.session_state.get("current_role", "")).strip().lower() != "admin":
        return
    if not isinstance(payroll_df, pd.DataFrame) or payroll_df.empty:
        return

    safe_batch = re.sub(r"[^A-Za-z0-9_-]+", "_", str(batch))
    ...
    with st.expander("⏭️ Tạm hoãn Vi phạm sang kỳ kế tiếp", expanded=False):
        ...
        _due_map, _deferred_map, _active_debts = get_violation_debt_state(
            start_date,
            end_date,
            payroll_df["Tên Hệ thống"].astype(str).tolist(),
        )

        _options = []
        _available_by_emp = {}
        for _emp in payroll_df["Tên Hệ thống"].astype(str).tolist():
            _ek = normalize_login_name(_emp)
            _raw_current = max(
                0.0, float(_money_to_float(_raw_penalty_map.get(_ek, 0)))
            )
            _already_deferred = max(
                0.0, float(_money_to_float(_deferred_map.get(_ek, 0)))
            )
            _available = max(0.0, _raw_current - _already_deferred)
            if _available > 0:
                _options.append(_emp)
                _available_by_emp[_emp] = (
                    _raw_current,
                    _already_deferred,
                    _available,
                )

        _options = sort_employee_names(_options)
        if _options:
            _emp = st.selectbox(
                "Nhân viên",
                _options,
                filter_mode="contains",
                key=f"hist_defer_emp_{safe_batch}",
            )
            _raw_current, _already_deferred, _available = _available_by_emp[_emp]

            _c1, _c2, _c3 = st.columns(3)
            _c1.metric("Vi phạm gốc kỳ này", f"{_raw_current:,.0f} đ".replace(",", "."))
            _c2.metric("Đã tạm hoãn", f"{_already_deferred:,.0f} đ".replace(",", "."))
            _c3.metric("Còn có thể hoãn", f"{_available:,.0f} đ".replace(",", "."))
            ...
            _amount = st.number_input(
                ...
                min_value=0.0,
                max_value=float(_available),
                value=float(_available),
                step=float(_step),
                format="%.0f",
                key=f"hist_defer_amount_{safe_batch}_{normalize_login_name(_emp)}",
            )

            if st.button(
                "💾 Lưu nghĩa vụ Vi phạm sang kỳ kế tiếp",
                use_container_width=True,
                key=f"hist_save_deferred_{safe_batch}",
            ):
                ...
                if ok:
                    # V87.7 fallback tức thời: giảm trực tiếp cột Vi phạm của nhân viên
                    # theo số vừa tạm hoãn, sau đó helper sẽ đối chiếu lại ledger tươi.
                    _local_after_defer = payroll_df.copy()
                    _local_mask = _local_after_defer.get(
                        "Tên Hệ thống", pd.Series("", index=_local_after_defer.index)
                    ).astype(str).apply(normalize_login_name).eq(
                        normalize_login_name(_emp)
                    )
                    if _local_mask.any() and "Tiền phạt trong tháng" in _local_after_defer.columns:
                        _local_idx = _local_after_defer.index[_local_mask][0]
                        _local_after_defer.at[_local_idx, "Tiền phạt trong tháng"] = max(
                            0.0,
                            float(_money_to_float(_local_after_defer.at[_local_idx, "Tiền phạt trong tháng"]))
                            - float(_money_to_float(_amount)),
                        )
                        _local_after_defer = recalculate_payroll_net(_local_after_defer)

                    _history_refresh_after_violation_change(
                        _local_after_defer,
                        start_date,
                        end_date,
                        hist_refresh_key,
                    ...
                    st.success(
                        msg
                        + " Bản lương lịch sử đang mở đã được tính lại; "
                          "hãy kiểm tra rồi bấm Ghi đè nếu muốn lưu snapshot mới."
                    )
                    rerun_current_view()
                else:
                    st.error(msg)
        else:
            st.info("Không còn khoản Vi phạm của kỳ này có thể tạm hoãn.")

        # Cùng bộ quản lý nghĩa vụ đang mở như tab Tính lương.
        _all_active = get_all_open_violation_debts_for_admin()
        if isinstance(_all_active, pd.DataFrame) and not _all_active.empty:
            st.markdown("#### 📋 Nghĩa vụ Vi phạm đang mở")
            _show_cols = [
                c for c in [
                    "Tên nhân viên", "Số tiền", "Nội dung", "Loại",
                    "Kỳ phát sinh từ", "Kỳ phát sinh đến",
                    "Bắt đầu trừ từ", "Trạng thái"
                ]
                if c in _all_active.columns
            ]
            _show = _all_active[_show_cols].copy()
            if "Số tiền" in _show.columns:
                _show["Số tiền"] = _show["Số tiền"].apply(
                    lambda x: f"{_money_to_float(x):,.0f}".replace(",", ".")
                )
            st.dataframe(_show, hide_index=True, width="stretch", height="content")

            with st.expander("✏️ Sửa / Xóa Nghĩa vụ Vi phạm", expanded=False):
                _rows = _all_active.reset_index(drop=True).copy()
                _options_manage = []
                _lookup = {}

                for _, _r in _rows.iterrows():
                    _sheet_row = int(_r.get("__sheet_row", 0) or 0)
                    _emp_name = str(_r.get("Tên nhân viên", "")).strip()
                    ...
                    _label = (
                        f"{_emp_name} · {_amt:,.0f} đ · {_typ} · {_p1} → {_p2}"
                    ).replace(",", ".")
                    if _label in _lookup:
                        _label = f"{_label} · dòng {_sheet_row}"
                    _options_manage.append(_label)
                ...
                _selected_label = st.selectbox(
                    ...
                    _options_manage,
                    filter_mode="contains",
                    key=f"hist_manage_debt_select_{safe_batch}",
                )
                _selected = _lookup.get(_selected_label, {})
                _selected_row = int(_selected.get("__sheet_row", 0) or 0)
                _selected_amount = max(
                    0.0, float(_money_to_float(_selected.get("Số tiền", 0)))
                )
                _selected_due = (
                    _parse_vn_date(_selected.get("Bắt đầu trừ từ", ""))
                    or start_date
                )
                ...
                _edit_amount = st.number_input(
                    ...
                    step=50000.0,
                    format="%.0f",
                    key=f"hist_edit_debt_amount_{safe_batch}_{_selected_row}",
                )
                _edit_content = st.text_input(
                    "Nội dung",
                    value=str(_selected.get("Nội dung", "")).strip()
                    or VIOLATION_DEBT_CONTENT,
                    key=f"hist_edit_debt_content_{safe_batch}_{_selected_row}",
                )
                _edit_due = st.date_input(
                    "Bắt đầu trừ từ kỳ/ngày",
                    value=_selected_due,
                    format="DD/MM/YYYY",
                    key=f"hist_edit_debt_due_{safe_batch}_{_selected_row}",
                )
                ...
                if _btn_edit.button(
                    ...
                    use_container_width=True,
                    key=f"hist_save_debt_edit_{safe_batch}_{_selected_row}",
                ):
                    ...
                    if _ok:
                        ...
                        st.success(
                            _msg
                            + " Bản lịch sử đang mở đã được tính lại; "
                              "hãy kiểm tra trước khi Ghi đè."
                        )
                        rerun_current_view()
                    else:
                        st.error(_msg)
                ...
                _confirm_delete = st.checkbox(
                    ...
                    key=f"hist_confirm_delete_debt_{safe_batch}_{_selected_row}",
                )
                if _btn_delete.button(
                    "🗑️ Xóa nghĩa vụ",
                    use_container_width=True,
                    disabled=not _confirm_delete,
                    key=f"hist_delete_debt_{safe_batch}_{_selected_row}",
                ):
        ...
        st.caption(
            "Các thay đổi ở đây cập nhật sổ Nghĩa vụ Vi phạm ngay lập tức. "
            "Bảng lương lịch sử chỉ được ghi đè khi bạn bấm nút "
...
# V88.2 - Bảng quản trị gọn áp dụng toàn hệ thống.
# Cấu hình chi tiết vẫn có thể thay đổi tại Preset / UI Builder.
try:
    _compact_global_cfg, _ = load_compact_admin_preset()
except Exception:
    _compact_global_cfg = _compact_admin_preset_default()

_cgd = _compact_global_cfg.get("desktop", {})
_cgm = _compact_global_cfg.get("mobile", {})

st.markdown(f"""
<style id="vera-compact-admin-global-v882">
table {{
    border-collapse:separate!important;
    border-spacing:0!important;
    background:{_cgd.get('body_bg','#FFFFFF')}!important;
    border:{int(_cgd.get('border_width',1))}px solid {_cgd.get('border_color','#D9DEE5')}!important;
    border-radius:{int(_cgd.get('radius',10))}px!important;
    overflow:hidden!important;
    box-shadow:none!important;
}}
table th {{
    background:{_cgd.get('header_bg','#F1F3F5')}!important;
    color:{_cgd.get('header_text','#4A4F55')}!important;
    font-size:{int(_cgd.get('header_font_size',13))}px!important;
    font-weight:{_cgd.get('header_weight','600')}!important;
    padding:{int(_cgd.get('cell_padding_y',5))}px {int(_cgd.get('cell_padding_x',8))}px!important;
    border-color:{_cgd.get('border_color','#D9DEE5')}!important;
}}
table td {{
    background:{_cgd.get('body_bg','#FFFFFF')}!important;
    color:{_cgd.get('body_text','#2F3337')}!important;
    font-size:{int(_cgd.get('font_size',13))}px!important;
    padding:{int(_cgd.get('cell_padding_y',5))}px {int(_cgd.get('cell_padding_x',8))}px!important;
    border-color:{_cgd.get('border_color','#D9DEE5')}!important;
}}
table tbody tr:nth-child(even) td {{
    background:{_cgd.get('zebra_bg','#FCFCFD')}!important;
}}
table tbody tr:hover td {{
    background:{_cgd.get('row_hover_bg','#F8FAFC')}!important;
}}

[data-testid="stExpander"] details > summary {{
    background:{_cgd.get('expander_header_bg','#E9ECEF')}!important;
}}
[data-testid="stExpander"] details > summary p {{
    font-size:{int(_cgd.get('expander_font',15))}px!important;
    font-weight:700!important;
}}
...
(function(){{
 try {{
  const W=window.parent,D=W.document,labels={_save_labels_json};
...
def has_page_access(page_name):
    ...
    if page_name == "🏖️ Lịch nghỉ dài hạn":
        _role = str(st.session_state.get("current_role", "")).strip().lower()
        if _role == "admin":
            return True
        return (
            has_feature_access("long_leave_form")
            or has_feature_access("long_leave_stats")
        )
...
# ==========================================================
# V90.1 - THÔNG BÁO CLICK ĐỂ MỞ ĐÚNG KHU VỰC
# ==========================================================
def render_clickable_app_notice(message, page_name, notice_key="", kind="warning"):
    """
    Render notification card có thể click.
    Click sẽ điều hướng trong cùng app bằng query params:
      ?page=<slug>&notice=<notice_key>
    """
    slug = str(PAGE_SLUGS.get(page_name, "") or "").strip()
    if not slug:
        # Fallback nếu page không có slug.
        if kind == "info":
            st.info(message)
        elif kind == "success":
            st.success(message)
    ...
    params = {"page": slug}
    if notice_key:
        params["notice"] = str(notice_key)
    href = "?" + _urlparse.urlencode(params)

    palette = {
        "warning": ("#FFF8E1", "#E6B800", "#5A4700"),
        "info": ("#EAF4FF", "#6AAAE8", "#153A5B"),
        "success": ("#EAF8EF", "#59B979", "#174A28"),
        "error": ("#FDECEC", "#E16A6A", "#6B1E1E"),
    }
    bg, border, color = palette.get(kind, palette["warning"])
    safe_message = html.escape(str(message))
    # V90.6: dùng flex thay cho float để thông báo dài trên mobile luôn tự xuống dòng,
    # không bị cắt phần đầu/cuối và mũi tên không đè lên nội dung.
    st.markdown(
        f"""
        <div class="vera-clickable-notice-wrap">
          <a class="vera-clickable-notice"
             href="{html.escape(href)}"
             style="--notice-bg:{bg};--notice-border:{border};--notice-color:{color};">
              <span class="vera-clickable-notice-text">{safe_message}</span>
              <span class="vera-clickable-notice-arrow">›</span>
          </a>
        </div>
        """,
        unsafe_allow_html=True,
...
# V90: thông báo nội bộ tới Admin + Quản lý + Lễ tân + Leader.
_ll_notice_role = str(st.session_state.get("current_role", "")).strip().lower()
if _ll_notice_role in {"admin", "quanly", "letan", "leader"}:
    try:
        _pending_ll_count = pending_long_leave_count()
...
if _ll_notice_role in {"quanly", "letan", "leader"}:
    try:
            ...
            if not _ll_my_decisions.empty and selected_page != "🏖️ Lịch nghỉ dài hạn":
                ...
                for _, _dec in _ll_my_decisions.head(3).iterrows():
                    ...
                    if _dec_status == LONG_LEAVE_APPROVED:
                        _dec_msg = (
                            f"✅ Đơn nghỉ dài hạn {_dec_id} đã được DUYỆT "
                        ...
                        _dec_msg = (
                            f"❌ Đơn nghỉ dài hạn {_dec_id} KHÔNG ĐƯỢC DUYỆT"
...
if selected_page == "📘 Hướng dẫn sử dụng":
    ...
    # V91.3: hai quyền độc lập.
    _can_use_long_leave_form = (
        _ll_role != "admin" and has_feature_access("long_leave_form")
    )
    _can_view_long_leave_stats = (
        _ll_role == "admin" or has_feature_access("long_leave_stats")
    )

    try:
        _ll_request_pause = load_long_leave_request_pause()
    except Exception:
        _ll_request_pause = {"enabled": False, "message": "Admin đang tạm dừng nhận đơn nghỉ dài hạn."}
    ...
    # ------------------------------------------------------
    if _ll_role == "admin":
        with st.expander("⏸️ Tạm dừng / Mở nhận đơn nghỉ dài hạn", expanded=False):
            ...
            _pause_toggle = st.toggle(
                "⏸️ Tạm dừng không cho tài khoản khác làm đơn",
                value=_pause_now,
    ...
    if _can_use_long_leave_form:
        with st.expander("📝 Tạo yêu cầu · Đơn xin nghỉ dài hạn", expanded=False):
            _monthly_req_count = count_employee_long_leave_requests_in_month(
                _ll_user, _ll_today
            )
            st.caption(
                f"Trong tháng {_ll_today.month:02d}/{_ll_today.year}: "
                f"đã gửi {_monthly_req_count}/5 đơn. "
                "Admin sẽ nhận thông báo trong app. Quản lý, Lễ tân và Leader cũng nhận "
                "thông báo; email của các vai trò này được CC nếu hồ sơ có Email."
            )
            ...
            if _monthly_req_count >= 5:
                st.error("❌ Bạn đã đạt giới hạn 5 đơn nghỉ dài hạn trong tháng này.")
            ...
            with st.form("long_leave_request_form_v90", clear_on_submit=False):
                ...
                _rq_submit = st.form_submit_button(
                    ...
                    disabled=(
                        ...
                        or _monthly_req_count >= 5
                    ),
                )

            if _rq_submit:
                if bool(_ll_request_pause.get("enabled", False)):
                    st.error("❌ Admin đang tạm dừng nhận đơn nghỉ dài hạn.")
                elif count_employee_long_leave_requests_in_month(_ll_user, _ll_today) >= 5:
                    st.error("❌ Mỗi người chỉ được gửi tối đa 5 đơn nghỉ dài hạn trong một tháng.")
        ...
        if isinstance(_ll_all, pd.DataFrame) and not _ll_all.empty:
            ...
            if not _my_req.empty:
                ...
                with st.expander(
                    ...
                    st.dataframe(
                        _my_req[[c for c in _my_cols if c in _my_req.columns]],
                        hide_index=True, width="stretch", height="content"
                    )
                    _my_labels, _my_lookup = [], {}
                    for _, _mr in _my_req.iterrows():
                        _ml = (
                            f"{_mr.get('ID','')} · {_mr.get('Từ ngày','')} → "
                            f"{_mr.get('Đến ngày','')} · {_mr.get('Trạng thái','')}"
                        )
                    ...
                    if _my_status == LONG_LEAVE_APPROVED:
                        ...
                        st.success(
                            "✅ ĐƠN ĐÃ ĐƯỢC DUYỆT\n\n"
                            f"Thời gian nghỉ: {_my_row.get('Từ ngày','')} → {_my_row.get('Đến ngày','')}\n\n"
                    ...
                    if (
                        ...
                        st.warning(
                            "🔔 Admin đã nhắc bạn tải tài liệu liên quan cho đơn này. "
                            "Bạn có thể tải ngay hoặc tải sau khi kết thúc thời gian nghỉ."
                        )
                    if _my_docs:
                    ...
                    _later_files = st.file_uploader(
                        ...
                        help=(
                            "Cho phép tải bổ sung trước, trong hoặc sau thời gian nghỉ dài hạn."
                        ),
                    )
                    if st.button(
                        "📤 Tải tài liệu lên",
                        ...
                        disabled=(not bool(_later_files) or not _can_upload_later),
                    ):
    ...
    # ------------------------------------------------------
    # Quản lý / Lễ tân / Leader: thông báo chi tiết đang chờ
    # ------------------------------------------------------
    if _ll_role in {"quanly", "letan", "leader"}:
        _notify_pending = (
            _ll_all[
        ...
        if not _notify_pending.empty:
            _ll_pending_view_open = _ll_active_notice == "long_leave_pending_view"
            if _ll_pending_view_open:
                render_notice_scroll_marker("ll-pending-view")
            with st.expander(
                f"🔔 Đơn nghỉ dài hạn đang chờ Admin duyệt ({len(_notify_pending)})",
                expanded=_ll_pending_view_open
            ):
                _notify_cols = [
                    "Tên nhân viên", "Vai trò", "Từ ngày", "Đến ngày",
                    "Lý do nghỉ dài hạn", "Chi tiết", "Ngày gửi"
                ]
                st.dataframe(
                    _notify_pending[
                        [c for c in _notify_cols if c in _notify_pending.columns]
                    ],
                    hide_index=True, width="stretch", height="content"
                )
        ...
        if not _notify_rejected.empty:
            ...
            with st.expander(
                ...
                for _, _rr in _notify_rejected.iterrows():
                    _lbl = (
                        f"{_rr.get('ID','')} · {_rr.get('Tên nhân viên','')} · "
                        f"{_rr.get('Từ ngày','')} → {_rr.get('Đến ngày','')}"
                    )
                ...
                if _rej_notice_id:
                    for _i, _lbl in enumerate(_rej_labels):
                        if _lbl.startswith(_rej_notice_id + " ·"):
                            _rej_default = _i
                            break
    ...
    if _ll_role == "admin":
        ...
        _ll_pending_admin_open = (
            _ll_active_notice == "long_leave_pending"
            or not _pending.empty
        )
        if _ll_active_notice == "long_leave_pending":
                ...
                with _c2:
                    if st.button(
                        ...
                        if not str(_reject_reason).strip():
                            st.error("❌ Bắt buộc nhập lý do Không duyệt nghỉ.")
                        else:
                            _now = datetime.now(VN_TZ)
                            _ok_up, _msg_up = update_long_leave_row(
                                int(_pr.get("__row", 0)),
        ...
        with st.expander(
            ...
            if _doc_candidates.empty:
                ...
                for _, _dr0 in _doc_candidates.iterrows():
                    _dl = (
                        f"{_dr0.get('Tên nhân viên','')} · {_dr0.get('ID','')} · "
                        f"{_dr0.get('Từ ngày','')} → {_dr0.get('Đến ngày','')}"
                    )
    ...
    if _can_view_long_leave_stats:
        ...
        st.info(
            "ℹ️ Tài khoản này được phép sử dụng mẫu Đơn xin nghỉ dài hạn "
    ...
    _shift_options_live = get_shift_options()
    shift_base = get_shift_dataframe(df_credentials)

    # ======================================================
    # TỔNG HỢP SỐ LƯỢNG NHÂN VIÊN THEO CA
    # ======================================================
    ...
    if shift_base.empty:
        ...
        _summary_rows = []
        _summary_shift_values = list(_shift_options_live)
        _assigned_values = [
            x for x in shift_base["Ca làm việc"].astype(str).str.strip().unique().tolist()
            if x and x not in _summary_shift_values
        ]
        _summary_shift_values += _assigned_values
        for _shift_name in _summary_shift_values:
            _part = shift_base[shift_base["Ca làm việc"].astype(str).str.strip().eq(_shift_name)]
            row = {"Ca làm việc": _shift_name, "Tổng nhân viên": int(len(_part))}
            for dep in SHIFT_DEPARTMENT_ORDER:
                row[dep] = int((_part["Bộ phận"] == dep).sum())
            _summary_rows.append(row)
        _unassigned = shift_base[shift_base["Ca làm việc"].astype(str).str.strip().eq("")]
        if not _unassigned.empty:
            row = {"Ca làm việc": "Chưa phân ca", "Tổng nhân viên": int(len(_unassigned))}
            for dep in SHIFT_DEPARTMENT_ORDER:
                row[dep] = int((_unassigned["Bộ phận"] == dep).sum())
            _summary_rows.append(row)
        st.dataframe(
    ...
    # ======================================================
    # QUẢN LÝ DANH MỤC CA - ADMIN
    # ======================================================
    if st.session_state.current_role == "admin":
        with st.expander("🛠️ Tạo / Sửa / Xóa ca làm việc", expanded=False):
            _defs = load_shift_definitions()
            _def_labels = ["➕ Tạo ca mới"]
            _def_lookup = {}
            if isinstance(_defs, pd.DataFrame) and not _defs.empty:
                for _, rr in _defs[_defs["Trạng thái"].astype(str).str.lower().ne("đã xóa")].iterrows():
                    lbl = _shift_display_label(rr["Tên ca"], rr["Giờ bắt đầu"], rr["Giờ kết thúc"])
                    if str(rr.get("Ghi chú","")).strip().lower() == "không đổi" and "không đổi" not in lbl.lower():
                        lbl += " (Không đổi)"
                    full = f"{rr['ID']} · {lbl}"
                    _def_labels.append(full)
                    _def_lookup[full] = rr.to_dict()

            _def_pick = st.selectbox(
                "Chọn ca cần sửa hoặc tạo mới",
                _def_labels,
                key="shift_definition_pick_v912",
            )
            _selected_def = _def_lookup.get(_def_pick, {})
            _is_new_shift = _def_pick == "➕ Tạo ca mới"

            sc1, sc2, sc3 = st.columns([1.6, 1, 1])
            with sc1:
                _shift_name_input = st.text_input(
                    "Tên ca",
                    value="" if _is_new_shift else str(_selected_def.get("Tên ca","")),
                    key=f"shift_def_name_{_def_pick}",
                    placeholder="VD: Ca 3",
                )
            with sc2:
                _shift_start_input = st.text_input(
                    "Giờ bắt đầu",
                    value="" if _is_new_shift else str(_selected_def.get("Giờ bắt đầu","")),
                    key=f"shift_def_start_{_def_pick}",
                    placeholder="14:00",
                )
            with sc3:
                _shift_end_input = st.text_input(
                    "Giờ kết thúc",
                    value="" if _is_new_shift else str(_selected_def.get("Giờ kết thúc","")),
                    key=f"shift_def_end_{_def_pick}",
                    placeholder="00:00",
                )
            sd1, sd2 = st.columns([2,1])
            with sd1:
                _shift_note_input = st.text_input(
                    "Ghi chú",
                    value="" if _is_new_shift else str(_selected_def.get("Ghi chú","")),
                    key=f"shift_def_note_{_def_pick}",
                    placeholder="VD: Không đổi",
                )
            with sd2:
                _shift_order_input = st.number_input(
                    "Thứ tự",
                    min_value=1,
                    max_value=999,
                    value=(
                        int(_selected_def.get("Thứ tự", len(_def_labels)))
                        if not _is_new_shift else len(_def_labels)
                    ),
                    step=1,
                    key=f"shift_def_order_{_def_pick}",
                )
            ...
            with sb1:
                if st.button(
                    "💾 Lưu ca làm việc",
                    use_container_width=True,
                    key="save_shift_definition_v912",
                ):
                    ok, msg = save_shift_definition(
                        "" if _is_new_shift else _selected_def.get("ID",""),
                        _shift_name_input,
                        _shift_start_input,
                        _shift_end_input,
                        _shift_note_input,
            ...
            with sb2:
                _delete_confirm = st.checkbox(
                    "Xác nhận xóa ca",
                    disabled=_is_new_shift,
                    key=f"confirm_delete_shift_{_def_pick}",
                )
                if st.button(
                    "🗑️ Xóa ca làm việc",
                    use_container_width=True,
                    disabled=_is_new_shift or not _delete_confirm,
                    key="delete_shift_definition_v912",
                ):
                    ok, msg = delete_shift_definition(
                        _selected_def.get("ID",""),
                        st.session_state.current_user,
                    )
                    (st.success if ok else st.error)(msg)
                    if ok:
    ...
    st.info(
        "Bộ phận **Nhân viên + Leader** được hiển thị chung. "
        "Các bộ phận **Lễ tân, Quản lý, Locker, Tạp vụ** được tách riêng. "
        "Thay đổi trong bảng chỉ ghi Google Sheet khi bấm **Lưu Toàn Bộ Cấu Hình Ca**."
    )
    ...
    with c_export_shift:
        st.download_button(
            ...
            use_container_width=True,
            help="Template dùng danh mục ca hiện tại của hệ thống."
        )
    with c_import_shift:
        uploaded_shift = st.file_uploader(
            "📤 Import Excel template",
            type=["xlsx", "xlsm"],
            key="shift_template_upload",
            help="Dùng đúng template Export từ hệ thống."
        )
    ...
    st.markdown("#### 🔎 Bộ lọc")
    f1, f2, f3, f4 = st.columns([2.1, 1.45, 1.45, 1.45])
    with f1:
        filter_shift_name = st.text_input(
            "Tìm theo tên nhân viên",
            key="shift_filter_name",
            placeholder="Gõ tên để lọc..."
        )
    with f2:
        department_filter = st.selectbox(
            "Lọc theo bộ phận",
            ["- Tất cả bộ phận -"] + SHIFT_DEPARTMENT_ORDER,
            key="shift_filter_department_v912",
        )
    with f3:
        shift_filter_value = st.selectbox(
            "Lọc theo ca",
            ["- Tất cả ca -"] + _shift_options_live,
            key="shift_filter_shift"
        )
    with f4:
        cycle_filter_value = st.selectbox(
            "Lọc theo luân phiên",
            ["- Tất cả luân phiên -"] + SHIFT_CYCLE_OPTIONS,
            key="shift_filter_cycle"
        )
    ...
    if str(filter_shift_name).strip():
        ...
        filtered_shift = filtered_shift[
            filtered_shift['Tên nhân viên'].astype(str)
            .apply(normalize_login_name).str.contains(keyq, regex=False)
        ]
    if department_filter != "- Tất cả bộ phận -":
        filtered_shift = filtered_shift[filtered_shift["Bộ phận"].eq(department_filter)]
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Ca làm việc": st.column_config.SelectboxColumn(
                        "Ca làm việc", options=_shift_options_live, width="large"
                    ),
                ...
                hide_index=True,
                use_container_width=True,
                key="shift_schedule_editor_form"
            )
            submit_shift = st.form_submit_button(
                "💾 Lưu Toàn Bộ Cấu Hình Ca",
                use_container_width=True
            )
    ...
    with st.expander("👥 Quyền mặc định theo loại tài khoản", expanded=True):
        ...
        _leave_edit_permission_keys = {
            "leave_detail_edit",
            "leave_manage_edit",
            "leave_today_khong_phep_edit_delete",
        }
        _account_only_permission_keys = {"employee_penalty_view"}
        _hidden_parent_permission_keys = {"long_leave"}
        _role_visible_feature_keys = [
            k for k in feature_keys
            if k not in _account_only_permission_keys
            and k not in _hidden_parent_permission_keys
            and (k not in _leave_edit_permission_keys or role_choice in {"quanly", "letan"})
        ]
        _role_visible_feature_labels = [FEATURE_DEFINITIONS[k] for k in _role_visible_feature_keys]
        current_role_allowed = [
            FEATURE_DEFINITIONS[k] for k in _role_visible_feature_keys
            if has_feature_access(k, role=role_choice, username='')
        ]
        role_allowed_labels = st.multiselect(
            "Các chức năng được phép", _role_visible_feature_labels, default=current_role_allowed,
    ...
    with st.expander("👤 Quyền riêng theo từng tài khoản", expanded=False):
        ...
        if account_choice:
            ...
            _account_visible_feature_keys = []
            for k in feature_keys:
                if k == "long_leave":
                    continue
                if k == "employee_penalty_view":
                    if account_role == "quanly":
                        _account_visible_feature_keys.append(k)
                    continue
                if k in {
                    "leave_detail_edit",
                    "leave_manage_edit",
                    "leave_today_khong_phep_edit_delete",
                } and account_role not in {"quanly", "letan"}:
                    continue
                _account_visible_feature_keys.append(k)
            _account_visible_feature_labels = [FEATURE_DEFINITIONS[k] for k in _account_visible_feature_keys]
            if has_account_override:
                current_account_allowed = [
                    FEATURE_DEFINITIONS[k] for k in _account_visible_feature_keys
                    if _account_cfg.get((normalize_login_name(account_choice), k), False)
                ]
            else:
                current_account_allowed = [
                    FEATURE_DEFINITIONS[k] for k in _account_visible_feature_keys
                    if has_feature_access(k, role=account_role, username='')
                ]
            ...
            account_allowed_labels = st.multiselect(
                "Các chức năng cho tài khoản này", _account_visible_feature_labels, default=current_account_allowed,
    ...
    st.caption(
        ...
        "Trạng thái được lưu trên Google Sheet nên vẫn giữ nguyên sau deploy/restart. "
        "Tài khoản Admin luôn được mở quyền đăng ký và không thể bị khóa."
    )
    ...
    st.info(
        "🛡️ Khóa tại trang này chỉ tác động đến ĐĂNG KÝ lịch nghỉ. "
        ...
        if not has_feature_access("payroll_history"):
            ...
            if history.empty or 'Mã bản lưu' not in history.columns:
                ...
                if not saved.empty:
                    ...
                    # V87.7: ngay khi mở lịch sử lần đầu, đối chiếu riêng 2 cột Vi phạm
                    # với sổ NoViPham hiện tại. Nhờ vậy bản cũ đã có khoản tạm hoãn
                    # sẽ hiển thị ngay số thực tế phải trừ, không cần bấm Cập nhật hệ thống.
                    if hist_refresh_key not in st.session_state:
                        try:
                            _hist_leave_live = load_backup_sheet_data()
                            _hist_violation_ledger = _load_violation_debt_ledger_from_sheets()
                            saved_table = apply_live_violation_state_to_payroll(
                                saved_table,
                                hs,
                                he,
                                leave_primary=_hist_leave_live,
                                violation_ledger_df=_hist_violation_ledger,
                            )
                            st.session_state[hist_refresh_key] = saved_table.copy()
                        except Exception:
                            pass
                    ...
                    # V87.4: Lịch sử bảng lương có đầy đủ công cụ Nghĩa vụ Vi phạm như tab Tính lương.
                    if str(st.session_state.get("current_role", "")).strip().lower() == "admin":
                        st.markdown("#### 💳 Nghĩa vụ Vi phạm / Nợ chuyển kỳ")
                        render_admin_open_violation_debt_overview(
                            key_prefix=f"history_{batch}"
                        )
                        render_admin_history_violation_management(
                            edited_saved_table,
                            hs,
                            he,
                            batch,
    ...
    if _leave_reset_pending:
        ...
        for _reset_key in list(st.session_state.keys()):
            if str(_reset_key).startswith("num_songay_") or str(_reset_key).startswith("num_phat_"):
                st.session_state.pop(_reset_key, None)
        ...
        if _registration_locked and not is_admin_leave_registration:
            ...
            if chosen_loai and chosen_loai != "-- Chọn lý do nghỉ --":
                ...
                if num_days_temp > 1 and "phép năm" not in norm_loai_temp:
                    ...
                    _early_all_sources = combine_leave_sources_for_daily_stats(
                    ...
                    for i in range(num_days_temp):
                        ...
                        # Giữ khung giờ riêng cho đúng lý do "Nghỉ phát sinh" như quy định cũ.
            ...
            with st.form("form_nhap_lich_inner"):
                ...
                input_chitiet = st.text_input(
                    txt_chitiet_label,
                    key="leave_reg_detail_input_v884"
                ).strip()
                ...
                # V88.4: không hiện cảnh báo khi nhân viên đã có một lịch KHÁC loại trong cùng ngày.
                # Chỉ tiếp tục chặn đúng Lý do nghỉ bị trùng đối với non-admin.
                confirm_multiple = True
                if existing_today and not is_admin_leave_registration:
                    ...
                    if normalize_leave_reason(chosen_loai) in normalized_existing:
                        st.error(
                            f"❌ Nhân viên này đã có Lý do nghỉ: "
                            f"'{clean_leave_reason_display(chosen_loai)}' vào ngày này rồi. "
                            "KHÔNG THỂ trùng cùng 1 loại nghỉ!"
                        )
                ...
                if submit_lich:
                    ...
                    if can_proceed:
                        if not confirm_multiple:
                            ...
                            if can_proceed:
                                _weekend_all_sources = combine_leave_sources_for_daily_stats(
                                    df_lich, df_leave_secondary, df_backup
                                )
                                _weekend_ok, _weekend_msg = _validate_monthly_weekend_registration_limit(
                                    _weekend_all_sources,
                                    chosen_nv,
                                    start_date,
                                    end_date,
                                    max_weekend_dates=2,
                                ...
                                if not _weekend_ok:
                                    st.error(f"❌ {_weekend_msg}")
                                    can_proceed = False

                            # KIỂM TRA GIỚI HẠN NHÂN SỰ CÁ NHÂN
                            # Admin vẫn bỏ qua các hạn mức phép/phát sinh khác,
                            # nhưng quy tắc tối đa 2 lần cuối tuần/tháng là quy tắc theo NHÂN VIÊN.
                            if can_proceed:
                                ...
                                if can_proceed:
                                    ...
                                    for i in range(num_days_selected):
                                        ...
                                        if (not is_admin_leave_registration
                                            ...
                                            # Giữ khung giờ cũ cho đúng lý do "Nghỉ phát sinh".
                                            ...
                                            latest_quota_df = combine_leave_sources_for_daily_stats(
                                                df_lich, df_leave_secondary, df_backup
                                            )
    ...
    # V89: Admin hoặc tài khoản quanly được cấp quyền riêng mới xem tiền phạt.
    is_admin = str(st.session_state.get("current_role", "")).strip().lower() == "admin"
    can_view_penalty = can_view_employee_penalty()
    if can_view_penalty:
        st.write("")
    ...
    if not daily_thuc_nghi.empty:
        ...
        _daily_dynamic_rules.extend([
            ".vera-daily-summary-wrap{overflow-x:hidden!important;width:100%!important;max-width:100%!important;}",
            ".vera-daily-summary-table{"
            "min-width:0!important;width:100%!important;max-width:100%!important;"
            "table-layout:fixed!important;"
            f"border-spacing:0 {int(_daily_ms['row_gap'])}px!important;"
            "}",
            ".vera-daily-summary-table th{"
            f"font-size:{int(_daily_ms['header_font'])}px!important;"
            f"padding:{int(_daily_ms['padding_y'])}px {int(_daily_ms['padding_x'])}px!important;"
            "white-space:normal!important;overflow-wrap:anywhere!important;"
            "}",
            ".vera-daily-summary-table td{"
            f"font-size:{int(_daily_ms['body_font'])}px!important;"
            f"padding:{int(_daily_ms['padding_y'])}px {int(_daily_ms['padding_x'])}px!important;"
            "white-space:normal!important;overflow-wrap:anywhere!important;"
            "}",
            ".vera-daily-summary-pill{"
            "display:block!important;width:100%!important;min-width:0!important;max-width:100%!important;"
            f"font-size:{int(_daily_ms['pill_font'])}px!important;"
            f"padding:{int(_daily_ms['pill_padding_y'])}px 0!important;"
            f"border-radius:{int(_daily_ms['pill_radius'])}px!important;"
            "white-space:normal!important;"
            "}",
        ])
    ...
    if df_backup_view.empty:
        ...
        if not can_view_employee_penalty() and "Phạt vi phạm" in manage_visible.columns:
        ...
        _manage_has_general_edit = has_feature_access("leave_manage_edit")
        _manage_has_today_khong_phep = has_feature_access("leave_today_khong_phep_edit_delete")
        _manage_role_permission_locked = (
            st.session_state.current_role in {"letan", "quanly"}
            and not (_manage_has_general_edit or _manage_has_today_khong_phep)
        )
        _manage_employee_system_locked = (
            st.session_state.current_role in EMPLOYEE_LIKE_ROLES
            and system_status['lock_nv']
        )
        manage_locked = _manage_role_permission_locked or _manage_employee_system_locked

        if _manage_role_permission_locked:
            st.info(
                "🔒 Tài khoản hiện chỉ có quyền XEM bảng Chi tiết danh sách tại Quản lý lịch nghỉ. "
                "Admin có thể mở quyền sửa tại Phân quyền chức năng."
            )
            manage_derived = list(manage_visible.columns)
        elif (
            st.session_state.current_role in {"letan", "quanly"}
            and _manage_has_today_khong_phep
            and not _manage_has_general_edit
        ):
            st.info(
                "🗓️ Tài khoản chỉ được sửa/xóa dòng thuộc nhóm KHÔNG PHÉP của đúng ngày hiện tại. "
                "Các thay đổi khác sẽ bị từ chối khi Lưu."
            )
        ...
        if not can_view_employee_penalty() and 'Phạt vi phạm' in manage_original_visible.columns:
        ...
        if manage_submit_save:
            if _manage_role_permission_locked:
                st.error("❌ Tài khoản này chưa được cấp quyền sửa trực tiếp bảng Quản lý lịch nghỉ.")
            elif not manage_changed:
                st.info('Không có thay đổi nào cần lưu.')
            else:
                all_ok = True
                save_messages = []
