# V86.3 - Bỏ toàn bộ conditional formatting bảng lịch nghỉ + phân nhóm theo Loại nghỉ B→C + V86.2 features (2026-08-19)
...
# ==========================================
if selected_page == "🎂 Sinh nhật nhân sự" and has_feature_access("birthday"):
    ...
    with leave_registration_area:
        ...
        if st.session_state.current_role in EMPLOYEE_LIKE_ROLES and system_status["lock_nv"]:
            ...
            if chosen_loai and chosen_loai != "-- Chọn lý do nghỉ --":
                ...
                elif (not is_special_day_exempt_temp and not is_nghi_ly_do_khac and default_phat <= 0
                    ...
                    if can_proceed:
                        if not confirm_multiple:
                            ...
                            if can_proceed:
                                ...
                                if is_video_leave:
                                    # Miễn Phép năm / Phát sinh / Có phép tháng và mọi giới hạn cá nhân.
                                    pass
                                ...
                                if can_proceed:
                                    ...
                                    for i in range(num_days_selected):
                                        ...
                                        # Chỉ chặn CÙNG lý do trong cùng ngày. Nếu Phạt vi phạm > 0,
                                        # cùng nhân viên vẫn được có nhiều dòng trong ngày miễn là lý do khác nhau.
                                        ...
                                        if (not special_day_exempt_save and not is_nghi_ly_do_khac and val_phat <= 0
    ...
    if stats_filtered_df.empty:
        ...
        if df_thuc_nghi.empty:
            ...
            # V86.3: phân nhóm theo Loại nghỉ ở cột C của sheet LoaiNghi,
            # đối chiếu bằng Lý do nghỉ ở cột B. Không suy đoán từ chữ trong Lý do nghỉ.
            _type_norm = _leave_type_series_from_reason(df_thuc_nghi).astype(str).apply(normalize_login_name)
            phat_sinh_df = df_thuc_nghi[_type_norm.str.contains('phat sinh', na=False)].copy()
            khong_phep_df = df_thuc_nghi[_type_norm.str.contains('khong phep', na=False)].copy()
            co_phep_df = df_thuc_nghi[_type_norm.str.contains('co phep', na=False)].copy()
    ...
    if not daily_thuc_nghi.empty:
        ...
        for d in sorted(daily_filtered_df['Ngày'].dropna().unique()):
            ...
            # V86.3: đếm CÓ phép / PHÁT SINH / KHÔNG phép theo cột C (Loại nghỉ) của LoaiNghi.
            d_type = _leave_type_series_from_reason(day_thuc_nghi).astype(str).apply(normalize_login_name)

            count_co_phep = int(d_type.str.contains('co phep', na=False).sum())
            count_phat_sinh = int(d_type.str.contains('phat sinh', na=False).sum())
            count_khong_phep = int(d_type.str.contains('khong phep', na=False).sum())
        ...
        st.caption("💡 Bấm vào số ở CÓ phép / PHÁT SINH / KHÔNG phép để xem Tên nhân viên và Lý do nghỉ.")
        _stat_widths = [1.35, 0.9, 1.15, 1.15, 1.15]
        if st.session_state.current_role == "admin":
            _stat_widths.append(1.25)

        _heads = st.columns(_stat_widths)
        _titles = ["Ngày", "Thứ ngày", "✅ CÓ phép", "⚠️ PHÁT SINH", "❌ KHÔNG phép"]
        if st.session_state.current_role == "admin":
        ...
        for _idx, _stat in daily_stats_df.iterrows():
            ...
            if not _day_thuc.empty:
                _tn = _leave_type_series_from_reason(_day_thuc).astype(str).apply(normalize_login_name)
                _co_rows = _day_thuc[_tn.str.contains('co phep', na=False)].copy()
                _ps_rows = _day_thuc[_tn.str.contains('phat sinh', na=False)].copy()
                _kp_rows = _day_thuc[_tn.str.contains('khong phep', na=False)].copy()
            else:
            ...
            if _rc[2].button(str(_co_n), key=f"daily_co_{_idx}_{_day_label}", use_container_width=True, disabled=_co_n <= 0):
                show_daily_leave_popup(_day_label, "CÓ phép", _co_rows)
            if _rc[3].button(str(_ps_n), key=f"daily_ps_{_idx}_{_day_label}", use_container_width=True, disabled=_ps_n <= 0):
                show_daily_leave_popup(_day_label, "PHÁT SINH", _ps_rows)
            if _rc[4].button(str(_kp_n), key=f"daily_kp_{_idx}_{_day_label}", use_container_width=True, disabled=_kp_n <= 0):
                show_daily_leave_popup(_day_label, "KHÔNG phép", _kp_rows)
            if st.session_state.current_role == "admin":
                _rc[5].markdown(str(_stat.get("💰 Tổng tiền phạt", "")))
