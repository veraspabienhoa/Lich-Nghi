# V86.7 - Sửa thống kê theo Lý do nghỉ + thêm Tổng nghỉ + V86.5 features (2026-08-19)
...
if selected_page == "📘 Hướng dẫn sử dụng":
    ...
    if stats_filtered_df.empty:
        ...
        if df_thuc_nghi.empty:
            ...
            phat_sinh_df = df_thuc_nghi[_reason_norm.str.contains('phat sinh', na=False)].copy()
            khong_phep_df = df_thuc_nghi[_reason_norm.str.contains('khong phep', na=False)].copy()
            co_phep_df = df_thuc_nghi[_reason_norm.str.contains('co phep', na=False)].copy()
    ...
    if not daily_thuc_nghi.empty:
        ...
        for d in sorted(daily_filtered_df['Ngày'].dropna().unique()):
            ...
            count_co_phep = int(d_reason.str.contains('co phep', na=False).sum())
            count_phat_sinh = int(d_reason.str.contains('phat sinh', na=False).sum())
            count_khong_phep = int(d_reason.str.contains('khong phep', na=False).sum())
        ...
        st.caption("💡 Thống kê theo nội dung cột Lý do nghỉ: chứa CÓ phép / PHÁT SINH / KHÔNG phép. Bấm vào số để xem chi tiết.")
        ...
        for _idx, _stat in daily_stats_df.iterrows():
            ...
            if not _day_thuc.empty:
                ...
                _co_rows = _day_thuc[_rn.str.contains('co phep', na=False)].copy()
                _ps_rows = _day_thuc[_rn.str.contains('phat sinh', na=False)].copy()
                _kp_rows = _day_thuc[_rn.str.contains('khong phep', na=False)].copy()
            else:
