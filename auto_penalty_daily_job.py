"""V86.16 - Auto Update tương thích Sheet1 A:K mới + email/retry V86.12.
...
# V86.16: ép nguồn TourVera mới cho mọi hàm tái sử dụng từ timesoft_sync_job.
...
SECONDARY_LEAVE_HEADERS = [
    ...
    "Số ngày phép cộng dồn", "Phạt vi phạm",
    "Ngày cập nhật", "Giờ cập nhật", "Người cập nhật",
]
...
def _sheet_rows_new(ws, source_id=None):
    """Đọc Sheet1 chính A:K; nguồn thứ hai vẫn A:J."""
    is_primary = source_id in {None, "", ts.SHEET_DU_PHONG_ID}
    headers = PRIMARY_LEAVE_HEADERS if is_primary else SECONDARY_LEAVE_HEADERS
    rng = "A:K" if is_primary else "A:J"
    ...
    for sheet_row, row in enumerate(values[1:], start=2):
        vals = list(row[:count]) + [""] * max(0, count - len(row))
        if not any(str(v).strip() for v in vals):
            continue
        item = dict(zip(headers, vals[:count]))
        if not is_primary:
            item["Loại nghỉ"] = ""
...
def _load_all_leave_rows_new(client):
    rows = []
    for sid in (ts.SHEET_DU_PHONG_ID, ts.SHEET_LICH_NGHI_2_ID):
        try:
            ws = client.open_by_key(sid).get_worksheet(0)
            for item in _sheet_rows_new(ws, sid):
                item["__source"] = sid
                rows.append(item)
        except Exception as exc:
            _log(f"WARN leave source {sid[:8]}: {type(exc).__name__}")

    logical = {}
    for item in rows:
        key = (
            ts._date_key(item.get("Ngày")),
            ts._employee_key(item.get("Tên nhân viên")),
            ts._reason_key(item.get("Lý do nghỉ")),
        )
        if all(key):
            logical[key] = item
...
def _save_auto_violation_new(client, d, employee, reason_item, detail, actor):
    """Bản tương thích V86.16: ghi Sheet1 mới A:K, cột D là Loại nghỉ."""
    reason = ts._clean_reason(reason_item.get("name", ""))
    ...
    current = primary_ws.get("A1:K1")
    header = current[0] if current else []
    if [str(x).strip() for x in header[:11]] != PRIMARY_LEAVE_HEADERS:
        primary_ws.update(
            range_name="A1:K1",
            values=[PRIMARY_LEAVE_HEADERS],
            value_input_option="USER_ENTERED",
        )

    live_rows = _load_all_leave_rows_new(client)
    if ts._same_leave_exists(live_rows, d, employee, reason):
        return True, "SKIP_DUPLICATE"

    accumulated = ts._monthly_accumulated_days(live_rows, d, employee, days)
    penalty = base_penalty
    canonical = ts._progressive_canonical(reason)
    if canonical:
        ordinal = ts._progressive_ordinal(live_rows, d, reason)
    ...
    target = _next_primary_row(primary_ws)
    primary_ws.update(
        range_name=f"A{target}:K{target}",
        values=[row],
        value_input_option="USER_ENTERED",
    )
    ...
    for d in sorted(affected_dates):
        ...
        for row in rows:
            ...
            if old_detail != new_detail or abs(old_penalty - new_penalty) > 0.1:
                primary_ws.update(
                    range_name=f"E{sheet_row}:H{sheet_row}",
                    values=[[
                        new_detail,
                        row.get("Số ngày tính", ""),
                        row.get("Số ngày phép cộng dồn", ""),
                        new_penalty,
                    ]],
                    value_input_option="USER_ENTERED",
                )
...
def _read_added_row(client, add_msg: str) -> dict:
    ...
    vals = ws.get(f"A{row_num}:K{row_num}")
    row = list(vals[0]) if vals else []
    row += [""] * max(0, 11 - len(row))
    out = {PRIMARY_LEAVE_HEADERS[i]: row[i] for i in range(11)}
    out["__row"] = row_num
    return out
