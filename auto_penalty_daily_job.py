"""V84.4 - Auto Update phạt hằng ngày lúc 20:00 + email thông báo.
...
- Nếu cùng ngày + cùng nhân viên có Lý do nghỉ chứa "Hỗ trợ": không tạo phạt TimeSoft.
...
DAILY_ACTOR_TS = "AUTO UPDATE 20:00 - TIMESOFT"
DAILY_ACTOR_TOUR = "AUTO UPDATE 20:00 - BẢNG TOUR"
EMAIL_LOG_HEADERS = [
...
def reverse_supported_timesoft_penalties(client, catalog: dict) -> dict:
    ...
    for row in primary_rows:
        ...
        has_support, support_reason = _has_support(supports, d, employee)
        if not has_support:
            continue
        sheet_row = int(row.get("__row", 0) or 0)
        if sheet_row >= 2:
...
def process_timesoft_today(client, cfg: dict, employee_map: dict, catalog: dict, supports: dict, checkin_df: pd.DataFrame) -> tuple[dict, list[dict]]:
    ...
    for _, row in checkin_df.iterrows():
        minutes = ts._parse_minutes_late(row)
        ...
        has_support, support_reason = _has_support(supports, work_date, employee)
        if has_support:
            result["skipped"] += 1
            result["support_skipped"] += 1
            _log(
                f"AUTO TIMESOFT SKIP SUPPORT: {employee} · {work_date.strftime('%d/%m/%Y')} · "
                f"{minutes:.0f} phút · '{support_reason}'"
            )
            continue
...
def run_daily() -> int:
    ...
    _log(f"Bắt đầu Auto Update 20:00; date={started.strftime('%Y-%m-%d')}")
    try:
        ...
        added_rows = ts_added + tour_added
        email_result = send_notifications(client, added_rows)

        _log(
            "Hoàn tất Auto Update 20:00: "
