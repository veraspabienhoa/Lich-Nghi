"""Vera Spa daily operations Cloud Run Job - V86.6.
...
SMTP_SENDER_EMAIL = (
    os.getenv("SMTP_SENDER_EMAIL", "veraspabienhoa@gmail.com") or ""
).strip()
...
def process_violations() -> str:
    ...
    for employee, rows in grouped.items():
        ...
        mail_results[employee] = send_email(
            to_email,
            cc,
            employee,
            target_date,
    ...
    sent_count = sum(
        1
        for ok, _ in mail_results.values()
        if ok
    )
