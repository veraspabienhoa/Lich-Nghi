"""Vera Spa daily operations Cloud Run Job - V86.16.
...
def append_violation(
    ...
    """Ghi Job 21:00 vào Sheet1 mới A:K; cột D lấy Loại nghỉ từ LoaiNghi."""
    ...
    # Bảo đảm header mới.
    current = ws.get("A1:K1")
    current_header = current[0] if current else []
    if [str(x).strip() for x in current_header[:11]] != PRIMARY_LEAVE_HEADERS:
        ws.update(
            "A1:K1",
            [PRIMARY_LEAVE_HEADERS],
            value_input_option="USER_ENTERED",
    ...
    ws.append_row(
        [
            date_text,                         # A Ngày
            employee,                          # B Tên nhân viên
            reason,                            # C Lý do nghỉ
            leave_type,                        # D Loại nghỉ
            detail,                            # E Chi tiết
            0,                                 # F Số ngày tính
            0,                                 # G Số ngày phép cộng dồn
            float(penalty or 0),               # H Phạt vi phạm
            now.strftime("%d/%m/%Y"),          # I Ngày cập nhật
            now.strftime("%H:%M:%S"),          # J Giờ cập nhật
            "AUTO UPDATE 21:00 - BẢNG TOUR",   # K Người cập nhật
        ],
        value_input_option="USER_ENTERED",
    )
    return True, "Đã ghi"
